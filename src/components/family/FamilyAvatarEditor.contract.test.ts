...
test('family avatar editor exposes only a complete avatar switch and photo mode', () => {
  ...
  assert.match(source, /'h-8 w-8' : 'h-11 w-11'/)
...
test('photo processing is progressive, cancellable, localized and blocks duplicate actions', () => {
  ...
  assert.match(source, /createAvatarPhotoPreview/)
  assert.match(source, /prepareAvatarPhoto\(crop\.file, selection\)/)
  assert.match(source, /BottomSheetSurface/)
  assert.match(source, /decodeImageAsset\(prepared, 'high'\)/)
  assert.match(source, /onPointerMove=\{moveCrop\}/)
})

test('cartoon and photo labels use the concise product copy', () => {
  assert.match(source, /cartoon: '卡通形象'/)
  assert.match(source, /photo: '照片'/)
})
