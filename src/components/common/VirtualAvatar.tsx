  ...
  return (
    <span className={`inline-flex shrink-0 overflow-hidden rounded-full bg-primary-soft ${className}`} role="img" aria-label={`${name}的虚拟卡通头像`}>
      <svg aria-hidden="true" viewBox="0 0 64 64" className="h-full w-full">
        ...
        <circle cx="27" cy={faceY + 2} r="1.25" fill="#334155" />
        <circle cx="38" cy={faceY + 2} r="1.25" fill="#334155" />
        <path d={`M28 ${faceY + 8}c2.5 2 5.5 2 8 0`} fill="none" stroke="#B86F63" strokeLinecap="round" strokeWidth="1.5" />
        {elder && (
          <g fill="none" stroke="#64748B" strokeWidth="1.2">
            <circle cx="27" cy={faceY + 2} r="4" />
            <circle cx="38" cy={faceY + 2} r="4" />
            <path d={`M31 ${faceY + 2}h3`} />
          </g>
        )}
