...
export function Header({ title, back, settings }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-20 grid min-h-14 grid-cols-[2.75rem_1fr_2.75rem] items-center border-b bg-surface/95 px-3 backdrop-blur">
      ...
      <div>{settings && <button className="grid h-11 w-11 place-items-center rounded-full hover:bg-primary-soft" onClick={() => navigate('/my')} aria-label="设置"><Settings size={19} /></button>}</div>
    </header>
  )
}
