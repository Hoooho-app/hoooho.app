import { ArrowLeft, Camera, ChevronDown, ImagePlus, Plus, Syringe, Trash2, X } from 'lucide-react'
...
const newItem = (): JournalVaccinationItem => ({ id: globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random()}`, vaccineName: '', doseSequence: 'unknown' })
const blankDraft = (): Draft => ({ items: [newItem()], institutionName: '', observations: [], note: '', occurredAt: localDateTimeValue(), recognitionStatus: 'not_used' })
const draftKey = (memberId: string) => `hoooho-vaccination-record-draft:${memberId}`
const recentKey = (memberId: string) => `hoooho-vaccination-recent:${memberId}`
const doses: Array<[VaccinationDose, string]> = [['dose_1', '第1剂'], ['dose_2', '第2剂'], ['dose_3', '第3剂'], ['dose_4', '第4剂'], ['booster', '加强剂'], ['unknown', '不清楚']]
const sites: Array<[VaccinationSite, string]> = [['left_upper_arm', '左上臂'], ['right_upper_arm', '右上臂'], ['left_thigh', '左大腿'], ['right_thigh', '右大腿'], ['other', '其他'], ['not_recorded', '未记录']]
const observationChoices: Array<[VaccinationObservation, string]> = [['not_observed_yet', '暂未观察'], ['nothing_notable', '没有特别表现'], ['injection_site_redness_or_pain', '接种处红肿或疼痛'], ['fever', '发热'], ['energy_or_appetite_change', '精神或食欲变化'], ['other', '其他']]
...
export function VaccinationRecordFlow({ memberId, token, selectedDay, today, onBack, onClose, onConfirm, onSaved }: { memberId: string; token: string; selectedDay: string; today: string; onBack: () => void; onClose: () => void; onConfirm: SaveRecord; onSaved: (message: string) => void }) {
  ...
  const chooseCertificate = (source: 'camera' | 'album', files: FileList | null) => { photos.chooseFiles(files); if (files?.length) setDraft((current) => ({ ...current, recognitionSource: source, recognitionStatus: current.items[0].vaccineName ? 'user_edited' : 'draft_unverified' })) }
  const toggleObservation = (value: VaccinationObservation) => setDraft((current) => {
    if (current.observations.includes(value)) return { ...current, observations: current.observations.filter((item) => item !== value) }
    if (value === 'not_observed_yet' || value === 'nothing_notable') return { ...current, observations: [value] }
    return { ...current, observations: [...current.observations.filter((item) => item !== 'not_observed_yet' && item !== 'nothing_notable'), value] }
  })
  const save = async () => {
  ...
  const supplemented = [draft.institutionName, draft.items.some((item) => item.manufacturerName), draft.items.some((item) => item.batchNumber), draft.items.some((item) => item.injectionSite), photos.photos.length, draft.observations.length, draft.note].filter(Boolean).length
  return <div className="medication-record-page-layer"><section aria-label="记录疫苗接种" aria-modal="true" className="medication-record-page vaccination-record-page" role="dialog">
    <header><button aria-label="返回记录新情况" disabled={saving} onClick={onBack} type="button"><ArrowLeft size={22} /></button><h1>记录疫苗接种</h1><button aria-label="关闭" disabled={saving} onClick={onClose} type="button"><X size={21} /></button></header>
    <div className="medication-record-scroll vaccination-record-scroll">
      {draft.items.map((item, index) => <section className="vaccination-item" key={item.id}><div className="vaccination-item-heading"><h2>{index === 0 ? '接种了什么疫苗？' : `同次接种的疫苗 ${index + 1}`}</h2>{index > 0 && <button aria-label={`删除疫苗 ${index + 1}`} onClick={() => setDraft((current) => ({ ...current, items: current.items.filter((row) => row.id !== item.id) }))} type="button"><Trash2 size={17} />删除</button>}</div><HohoInput label="疫苗名称" onChange={(event) => updateItem(item.id, { vaccineName: event.target.value })} placeholder="搜索疫苗名称，或手动填写" value={item.vaccineName} />{index === 0 && <><div className="medication-photo-actions"><button onClick={() => cameraRef.current?.click()} type="button"><Camera size={19} />拍接种凭证</button><button onClick={() => albumRef.current?.click()} type="button"><ImagePlus size={19} />从相册选择</button></div>{recent.length > 0 && <div className="vaccination-recent"><span>最近记录</span>{recent.map((row) => <button key={row.vaccineName} onClick={() => updateItem(item.id, { vaccineName: row.vaccineName, commonAbbreviation: row.commonAbbreviation, manufacturerName: row.manufacturerName })} type="button">{row.vaccineName}</button>)}</div>}</>}<fieldset className="medication-fieldset"><legend>第几剂？</legend><div className="medication-choice-grid">{doses.map(([value, label]) => <button aria-pressed={item.doseSequence === value} key={value} onClick={() => updateItem(item.id, { doseSequence: value })} type="button">{label}</button>)}</div></fieldset></section>)}
...
export { doseLabel, observationLabel }
