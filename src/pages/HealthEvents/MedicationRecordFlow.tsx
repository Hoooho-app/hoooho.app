...
export function MedicationRecordFlow({ memberId, token, selectedDay, today, initialMedication, initialOccurredAt, recordEditOnly = false, onBack, onClose, onConfirm, onSaved }: { memberId: string; token: string; selectedDay: string; today: string; initialMedication?: JournalMedicationDetails; initialOccurredAt?: string; recordEditOnly?: boolean; onBack: () => void; onClose: () => void; onConfirm: SaveRecord; onSaved: (message: string) => void }) {
  ...
  const closeRecord = (close:()=>void) => { if (recordEditOnly && JSON.stringify(draft)!==initialDraft.current) setConfirmExit(true); else close() }
  ...
  useEffect(() => { if (!recordEditOnly) sessionStorage.setItem(keyFor(memberId), JSON.stringify(draft)) }, [draft, memberId, recordEditOnly])
  ...
  return <div className="medication-record-page-layer"><section aria-label="记录用药" aria-modal="true" className="medication-record-page medication-record-compact" role="dialog">
    <header><button aria-label="返回记录新情况" disabled={saving} onClick={()=>closeRecord(onBack)} type="button"><ArrowLeft /></button><h1>记录用药</h1><button aria-label="关闭" disabled={saving} onClick={()=>closeRecord(onClose)} type="button"><X /></button></header>
    ...
    <div className="medication-record-scroll">
      ...
      {recordEditOnly ? <p>本次只修改已发生的服用记录。提醒计划与附件请在原入口管理。</p> : <section className="medication-reminder-main"><label><Bell /><strong>设置用药提醒</strong><input checked={reminder.enabled} onChange={(event) => { if (!event.target.checked) updateActive({ reminder: { ...reminder, enabled: false } }); else if (reminder.configured || active.reminder?.enabled) updateActive({ reminder: { ...reminder, enabled: true, configured: true } }); else openReminder() }} role="switch" type="checkbox" /></label>{reminder.enabled && <button className="medication-reminder-summary" onClick={openReminder} type="button"><span>{reminderSummary(reminder)}</span><em>编辑</em></button>}</section>}
    ...
    {confirmExit && <div role="alert" className="medication-compact-section"><p>还有未保存的用药修改</p><HohoButton loading={saving} onClick={()=>void save()}>保存</HohoButton><HohoButton variant="secondary" onClick={onClose}>放弃修改</HohoButton><HohoButton variant="text" onClick={()=>setConfirmExit(false)}>继续编辑</HohoButton></div>}
