...
export function HealthEventsPage() {
  ...
  const [today, setToday] = useState(() => getLocalDateKey(new Date())!); const [day, setDay] = useState(today); const [filterOpen, setFilterOpen] = useState(false); const [filters, setFilters] = useState<HealthEventFilters>(emptyHealthEventFilters); const [sortOrder, setSortOrder] = useState<'desc' | 'asc'>('desc'); const [recorderMode, setRecorderMode] = useState<'manual' | 'voice' | null>(null); const [nextActionOpen, setNextActionOpen] = useState(false); const [journalContext, setJournalContext] = useState<{ memberId: string; eventId: string | null }>({ memberId: currentMemberId, eventId: null }); const [revision, setRevision] = useState(0); const [savedNotice, setSavedNotice] = useState(''); const submissionKeyRef = useRef('')
