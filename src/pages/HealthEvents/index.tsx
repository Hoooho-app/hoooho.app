...
import type { JournalCategory, JournalMetadata, JournalSaveResult } from '../../types/journal'
...
import { JOURNAL_SAVED_FLASH_KEY } from './recordFlowTypes'
...
function takeSavedNotice() {
  const message = sessionStorage.getItem(JOURNAL_SAVED_FLASH_KEY) ?? ''
  sessionStorage.removeItem(JOURNAL_SAVED_FLASH_KEY)
  return message
}
function UserIdentity({ member, onSummary, summaryDisabled }: { member: Member | null; onSummary: () => void; summaryDisabled: boolean }) { const meta = member ? [genderLabels[member.gender ?? ''], member.age].filter(Boolean).join(' · ') : ''; return <div className="health-events-member mx-4 mt-2"><div className="journal-subject-row"><div className="journal-subject-card" aria-label="记录对象"><Avatar name={member?.name ?? ' '} src={member?.avatar} size="sm" /><span className="journal-subject-copy"><span className="journal-subject-name"><strong>{member?.name ?? ' '}</strong></span><span className="journal-subject-meta">{meta}</span></span></div><MedicalPrepButton aria-label="就医准备" className="journal-subject-summary" label="就诊情况单" onClick={onSummary} disabled={summaryDisabled} /></div></div> }
interface PendingDuplicate { duplicate: QuickRecordDuplicate; input: QuickRecordCreateInput; resolve: (result: JournalSaveResult) => void; reject: (reason: unknown) => void }
...
export function HealthEventsPage() {
  ...
  const [today, setToday] = useState(() => getLocalDateKey(new Date())!); const [day, setDay] = useState(() => returnState?.day && parsePlainDate(returnState.day) ? returnState.day : getLocalDateKey(new Date())!); const [sortOrder, setSortOrder] = useState<'desc' | 'asc'>('desc'); const [recorderMode, setRecorderMode] = useState<'manual' | 'voice' | null>(null); const [selectedRecord, setSelectedRecord] = useState<{ eventId: string; recordId: string } | null>(null); const [nextActionOpen, setNextActionOpen] = useState(false); const [pendingDuplicate, setPendingDuplicate] = useState<PendingDuplicate | null>(null); const [journalContext, setJournalContext] = useState<{ memberId: string; eventId: string | null }>({ memberId: currentMemberId, eventId: null }); const [revision, setRevision] = useState(0); const [savedNotice, setSavedNotice] = useState(takeSavedNotice); const submissionKeyRef = useRef(''); const contentRef = useRef<HTMLDivElement>(null)
  const [recorderInitialCategory, setRecorderInitialCategory] = useState<JournalCategory | undefined>()
  const [recorderCompleted, setRecorderCompleted] = useState(false)
  useEffect(() => {
  ...
  useEffect(() => {
    const closeCompletedRecorder = () => {
      setRecorderCompleted(true)
      setRecorderMode(null)
      setRecorderInitialCategory(undefined)
    }
    window.addEventListener('hoooho:journal-record-saved', closeCompletedRecorder)
    return () => window.removeEventListener('hoooho:journal-record-saved', closeCompletedRecorder)
  }, [])
  ...
  const saveJournalRecord = async (content: string, occurredAt: string, inputChannel: QuickRecordInputChannel, photos: QuickRecordPhotoPayload, journal: JournalMetadata) => {
    ...
    if (duplicate) return new Promise<JournalSaveResult>((resolve, reject) => setPendingDuplicate({ duplicate, input, resolve, reject }))
    const result = await quickRecordService.create(input, token)
    window.setTimeout(() => {
      setDay(getLocalDateKey(new Date(occurredAt))!)
      finishSave()
    }, 0)
    return { ...result, message: '已记录' }
  }
  const resolveDuplicate = async (action: 'update' | 'create', changeSummary?: string): Promise<void> => {
    ...
    try {
      const result = await quickRecordService.create({ ...pending.input, rawText: pending.input.content, content: action === 'update' && changeSummary ? changeSummary : pending.input.content, duplicateAction: action, duplicateEventId: pending.duplicate.eventId }, token)
      setDay(getLocalDateKey(new Date(pending.input.occurredAt))!); finishSave(); pending.resolve({ ...result, message: action === 'update' ? '已补充到原来的记录' : '已新增一条记录' })
    } catch (error) { pending.reject(error) }
  }
  const discardDuplicate = () => { if (!pendingDuplicate) return; const pending = pendingDuplicate; setPendingDuplicate(null); submissionKeyRef.current = ''; pending.resolve({ message: '已保留原来的记录', eventId: pending.duplicate.eventId, recordId: pending.duplicate.recordId, idempotent: true }) }
