...
type TimelineItem = TimelineItemBase & (
  ...
  | { kind: 'activity-routine'; activity: 'sleep' | 'meal'; track: RoutineTrack; projection: Exclude<IntervalProjection, 'open'>; durationMinutes: number; title: string }
  | { kind: 'empty' }
)
...
function CurrentTimeRow() {
  ...
  return <div className={`journal-timeline-row journal-timeline-row--now${reset ? ' journal-timeline-row--now-reset' : ''}`} data-time={clockLabel(now.getHours(), 0)}>
    <time dateTime={now.toISOString()}>{clockLabel(now.getHours(), 0)}</time><span aria-hidden="true" className="journal-timeline-marker"><span key={now.getSeconds()} /></span>
...
function RoutineRow({ track, onOpen }: { track: RoutineTrack; onOpen: () => void }) {
  return <button aria-label={`${track.title}，按作息`} className={`routine-grid-card${track.status === 'skipped' ? ' routine-grid-card--skipped' : ''}${track.status === 'confirmed' ? ' routine-grid-card--confirmed' : ''}`} data-routine-key={track.trackKey} onClick={onOpen} type="button"><Utensils aria-hidden="true" size={18} /><span className="routine-grid-copy"><strong>{track.title}</strong><small>· 按作息</small></span>{track.status === 'confirmed' ? <CheckCircle2 aria-label="已确认" className="routine-confirm-stamp" size={17} /> : <ChevronRight aria-hidden="true" size={16} />}</button>
}
...
function ActivityRow({ highlighted, item, onOpen }: { highlighted: boolean; item: Extract<TimelineItem, { kind: 'activity-record' | 'activity-routine' }>; onOpen: () => void }) {
  ...
  const isRoutine = item.kind === 'activity-routine'; const recordId = item.kind === 'activity-record' ? item.entry.id : undefined; const Icon = item.activity === 'sleep' ? Moon : Utensils
...
export function TimeView({ memberId, token, day, today, focusRecord, onFocusHandled, onDayChange, onRecordOpen, onRoutineRecorded, revision, onContext, sortOrder }: { memberId: string; token: string; day: string; today: string; focusRecord?: { recordId: string; day: string; revision: number; entry: JournalEntry } | null; onFocusHandled?: () => void; onDayChange: (day: string) => void; onRecordOpen: (eventId: string, recordId: string, options?: { correctSleep?: boolean }) => void; onRoutineRecorded?: () => void; revision: number; onContext: (context: { memberId: string; eventId: string | null }) => void; sortOrder: 'desc' | 'asc' }) {
  ...
  const displayItems = useMemo(() => {
    ...
    const addRoutine = (track: RoutineTrack) => { const interval = routineInterval(track); if (!interval) { const at = new Date(`${track.day}T${track.time}:00`); if (track.day === day && !(day === today && at > now)) items.push({ createdTime: at.getTime(), hour: at.getHours(), key: `routine:${track.trackKey}`, kind: 'routine', minute: at.getMinutes(), sortTime: at.getTime(), track }); return } if (track.recordId && actualActivityIds.has(track.recordId)) return; if (interval.start > now && day === today) return; const visibleEnd = day === today && now < interval.end ? now : interval.end; const includeEnd = visibleEnd.getTime() === interval.end.getTime(); for (const point of projectActivityInterval(interval.start, visibleEnd, day, includeEnd)) items.push({ activity: track.category === 'sleep' ? 'sleep' : 'meal', createdTime: interval.start.getTime(), durationMinutes: interval.durationMinutes, hour: point.hour, key: `routine-activity:${track.trackKey}:${point.key}`, kind: 'activity-routine', minute: point.minute, projection: point.kind, sortTime: point.at.getTime(), title: track.category === 'sleep' ? '睡眠' : track.title, track }) }
    ...
    for (const hour of orderedHours(localSortOrder).filter((candidate) => candidate <= lastHour)) if (!(day === today && hour === lastHour) && !items.some((item) => item.hour === hour && item.minute === 0)) { const at = new Date(`${day}T${String(hour).padStart(2, '0')}:00:00`); items.push({ createdTime: at.getTime(), hour, key: `empty:${hour}`, kind: 'empty', minute: 0, sortTime: at.getTime() }) }
  ...
  const currentAnchorIndex = day === today ? displayItems.findIndex((item) => localSortOrder === 'desc' ? item.hour < now.getHours() || (item.hour === now.getHours() && item.minute === 0) : item.hour >= now.getHours()) : -1
  ...
  if (day === today) timelineRows.splice(currentAnchorIndex === -1 ? timelineRows.length : currentAnchorIndex, 0, <CurrentTimeRow key="current-hour" />)
