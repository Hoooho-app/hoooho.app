...
import type { JournalBowelDetails } from '../../types/journal'
...
export function BowelRecordFlow({ memberId, token, linkedSymptomRecordIds = [], linkedBackfill, onBackfill, onBack, onClose, onConfirm, onSaved }: { memberId: string; token: string; linkedSymptomRecordIds?: string[]; linkedBackfill?: LinkedBackfillResult; onBackfill: (target: RecordBackfillTarget) => void; onBack: () => void; onClose: () => void; onConfirm: SaveJournalRecord; onSaved: (message: string) => void }) {
  ...
  return <div className="diet-record-page-layer"><section aria-label="记录排便" aria-modal="true" className="diet-record-page bowel-record-page" ref={layerRef} role="dialog" tabIndex={-1}>
    ...
    <div className="diet-record-scroll">
      ...
      <RecordRelationSection title="补充症状信息" hint="当时出现的身体表现" memberId={memberId} token={token} occurredAt={draft.occurredAt} categories={['symptom']} selectedIds={linkedSymptoms} onChange={setLinkedSymptoms} onBackfill={onBackfill} target="symptom" />
      <HohoInput label="实际排便时间" max={localDateTimeValue()} onChange={(event) => update('occurredAt', event.target.value)} type="datetime-local" value={draft.occurredAt} />
