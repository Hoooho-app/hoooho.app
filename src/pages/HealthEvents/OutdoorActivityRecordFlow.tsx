...
import { outdoorActivitySummary, toggleChoice, toggleExclusive } from './outdoorActivityLogic'
...
type Draft = JournalOutdoorActivityDetails & { occurredAt: string }
...
const emptyDraft = (): Draft => ({ activities: [], places: [], contacts: [], observations: [], occurredAt: localDateTimeValue() })
...
export function OutdoorActivityRecordFlow({ memberId, token, selectedDay, today, onBack, onClose, onConfirm, onSaved }: { memberId: string; token: string; selectedDay: string; today: string; onBack: () => void; onClose: () => void; onConfirm: SaveRecord; onSaved: (message: string) => void }) {
  const [draft, setDraft] = useState(() => readDraft(memberId)); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const layerRef = useRef<HTMLElement>(null); const photos = useQuickRecordPhotos(memberId, token, 6); const occurrence = useOccurrenceTime(selectedDay, today)
  ...
  const save = async () => { const occurredAt = occurrence.capture(); if (!occurredAt) return; setSaving(true); setError(''); try { const details: JournalOutdoorActivityDetails = { activities: draft.activities, places: [], contacts: draft.contacts, observations: draft.observations, ...(draft.activityOtherText?.trim() ? { activityOtherText: draft.activityOtherText.trim() } : {}), ...(draft.durationRange ? { durationRange: draft.durationRange } : {}), ...(draft.activityState ? { activityState: draft.activityState } : {}) }; const message = await onConfirm(outdoorActivitySummary(details), occurredAt, 'text', photos.payload(), { categories: ['activity'], outdoorActivity: details }); photos.clearAfterSave(); sessionStorage.removeItem(draftKey(memberId)); onSaved(message); onClose() } catch (reason) { setError(reason instanceof Error ? reason.message : '保存失败，请重试') } finally { setSaving(false) } }
  return <div className="diet-record-page-layer"><section aria-label="记录户外活动" aria-modal="true" className="diet-record-page outdoor-record-page" ref={layerRef} role="dialog" tabIndex={-1}><header><button aria-label="返回记录新情况" disabled={saving} onClick={onBack} type="button"><ArrowLeft size={22} /></button><h1>记录户外活动</h1><span aria-hidden="true" /></header><div className="diet-record-scroll outdoor-record-scroll">
    ...
    <fieldset className="outdoor-fieldset"><legend>大约活动了多久？<span>（可选）</span></legend><div className="outdoor-duration-grid">{durationOptions.map(([value, label]) => <button aria-pressed={draft.durationRange === value} key={value} onClick={() => update('durationRange', draft.durationRange === value ? undefined : value)} type="button">{label}</button>)}</div></fieldset>
    ...
    <OccurrenceTimeField model={occurrence} label="记录时间" />{error && <p className="diet-save-error" role="alert">{error}</p>}<div className="diet-record-save"><HohoButton disabled={saving || photos.blocked} fullWidth loading={saving} onClick={save} size="large">保存记录</HohoButton></div>
  </div></section></div>
}
