...
export interface SleepDraft extends JournalSleepDetails { kindWasChanged: boolean }
...
export function createSleepDraft(now = new Date()): SleepDraft {
  ...
  return { sleepAt: sleepAt.toISOString(), wakeAt: wakeAt.toISOString(), durationMinutes: 480, kind: defaultSleepType(sleepAt, wakeAt), kindWasChanged: false, observations: [] }
}
...
function SleepRing({ draft, onChange }: { draft: SleepDraft; onChange: (draft: SleepDraft) => void }) {
  ...
  const updateFromPointer = (event: ReactPointerEvent<SVGSVGElement>) => {
    ...
    onChange({ ...draft, sleepAt: range.start.toISOString(), wakeAt: range.end.toISOString(), durationMinutes: nextDuration, kind: draft.kindWasChanged ? draft.kind : defaultSleepType(range.start, range.end) })
  }
...
export function SleepRecordFlow({ draft, onDraftChange, onBack, onClose, onConfirm, onSaved }: { draft: SleepDraft; onDraftChange: (draft: SleepDraft) => void; onBack: () => void; onClose: () => void; onConfirm: SaveRecord; onSaved: (message: string) => void }) {
  ...
  const setClock = (field: 'sleep' | 'wake', value: string) => {
    ...
    onDraftChange({ ...draft, sleepAt: range.start.toISOString(), wakeAt: range.end.toISOString(), durationMinutes: length, kind: draft.kindWasChanged ? draft.kind : defaultSleepType(range.start, range.end) })
  }
  ...
  return <div className="sleep-record-page-layer"><section aria-label="记录睡眠" aria-modal="true" className="sleep-record-page" ref={layerRef} role="dialog" tabIndex={-1}>
    ...
    <div className="sleep-record-scroll"><SleepRing draft={draft} onChange={onDraftChange} />
      ...
      <fieldset className="sleep-fieldset"><legend>睡眠类型</legend><div className="sleep-choice-row sleep-kind-row">{([['night', '夜间睡眠'], ['nap', '白天小睡']] as const).map(([kind, label]) => <button aria-pressed={draft.kind === kind} key={kind} onClick={() => onDraftChange({ ...draft, kind, kindWasChanged: true })} type="button">{kind === 'night' ? <Moon size={17} /> : <Sun size={17} />}{label}</button>)}</div></fieldset>
