...
test('frame gate, stale-frame budget, save handoff, visibility and bounded recovery', (t) => {
  ...
  const videos = Array.from({ length: 3 }, () => new Video())
  const controller = createNursePlayback(videos as unknown as HTMLVideoElement[], ['intro', 'idle1', 'saved'])
  controller.setActive(true)
  assert.deepEqual(videos.map((v) => v.loads), [1, 0, 0])
  ...
  assert.deepEqual(videos.map((v) => v.loads), [1, 1, 0])
  now += 600; tick()
  assert.equal(videos[0].dataset.active, 'true')
  now += 800; tick()
  assert.equal(videos[0].dataset.active, 'false')
  ...
  videos[2].frame(0); videos[2].frame(0.04)
  assert.equal(videos[1].dataset.active, 'false')
  assert.equal(videos[2].dataset.active, 'true')
  controller.saved(1)
  assert.equal(videos[2].plays, 1)
  videos[2].finish()
  assert.equal(videos[2].dataset.active, 'false')
  videos[1].frame(0); videos[1].frame(0.04)
  assert.equal(videos[1].dataset.active, 'true')
  ...
  const failed = Array.from({ length: 3 }, () => new Video())
  const blocked = createNursePlayback(failed as unknown as HTMLVideoElement[], ['intro', 'idle1', 'saved'])
  blocked.setActive(true)
  for (let i = 0; i < 20; i++) { now += 8100; tick() }
  assert.deepEqual(failed.map((v) => v.plays), [3, 3, 0])
