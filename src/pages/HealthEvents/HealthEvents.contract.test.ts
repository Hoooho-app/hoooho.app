...
const occurrenceTime = read('./OccurrenceTimeField.tsx')
const occurrenceTimeModel = read('./occurrenceTimeModel.ts')
...
test('喂养饮食使用五类确认页、独立记录页和结构化真实保存', () => {
  ...
  assert.match(journalRecorder, /<JournalDietIcon kind="feeding" size=\{24\}/)
  assert.match(journalRecorder, /<JournalDietIcon kind="complementary" size=\{24\}/)
  assert.match(journalRecorder, /<JournalDietIcon kind="meal" size=\{24\}/)
  assert.match(journalRecorder, /<JournalDietIcon kind="snack" size=\{24\}/)
  assert.match(journalRecorder, /<JournalDietIcon kind="supplement" size=\{24\}/)
  ...
  assert.match(dietFlow, /<OccurrenceTimeField model=\{occurrence\} label="记录时间"/)
  ...
  assert.match(dietFlow, /const reactionOptions = \['皮肤', '呼吸', '消化'\]/)
  assert.doesNotMatch(dietFlow, /diet-no-reaction|onChange\(\['暂未发现'\]\)/)
  assert.match(dietFlow, /dietFrequentFoods/)
  assert.match(dietFlow, /添加常吃食物/)
  assert.match(dietFlow, /familyMemberService\.update/)
  assert.doesNotMatch(dietFlow, /<QuickRecordPhotos/)
  ...
  assert.match(occurrenceTime, />现在<\/button>/)
  assert.match(occurrenceTime, />指定时间<\/button>/)
...
test('健康随记日期栏保留单日导航且时间轨迹统一为紧凑小时格列表', () => {
  assert.doesNotMatch(timeView, /'day' \| 'month'|切换到月视图|切换到日视图|这个月还没有记录|shiftMonth/)
  assert.match(timeView, /const dayEntries = useMemo\(\(\) => entriesForDay\(timelineEntries, day, now\)/)
  assert.match(timeView, /focusRecord\?\.entry/)
  assert.match(timeView, /const displayItems = useMemo/)
  assert.match(timeView, /kind: 'empty'/)
  assert.match(timeView, /<CurrentTimeRow key="current-time" \/>/)
  assert.match(timeView, /timelineRows\.unshift\(currentRow\)/)
  assert.match(timeView, /timelineRows\.push\(currentRow\)/)
  ...
  assert.doesNotMatch(timeView, /缩略图视图|toggleLayoutMode|journal-hour-more|BottomSheetSurface/)
  assert.match(timeView, /sessionStorage\.removeItem\('hoooho:journal-layout'\)/)
  assert.match(timeView, /data-layout-mode="list"/)
  assert.match(timeView, /journal-timeline-row/)
  assert.match(timeView, /journal-hour-cell/)
...
test('听写与核对态共享同一照片草稿且不改变护士视频状态机', () => {
  ...
  assert.match(photos, /item\.status !== 'failed'/)
  assert.match(photos, /uploadVersionsRef/)
  assert.match(photos, /上传失败，请重试或移除/)
  assert.match(photos, /移除上传失败的照片/)
