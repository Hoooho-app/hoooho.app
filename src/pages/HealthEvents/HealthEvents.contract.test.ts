...
test('喂养饮食使用五类确认页、独立记录页和结构化真实保存', () => {
  ...
  assert.match(journalRecorder, /diet-types\/feeding\.webp/)
  assert.match(journalRecorder, /diet-types\/complementary\.webp/)
...
test('健康随记日期栏保留单日导航且时间轨迹统一为紧凑小时格列表', () => {
  ...
  assert.match(timeView, /kind: 'hour-divider'/)
  assert.match(timeView, /data-hour-divider=/)
