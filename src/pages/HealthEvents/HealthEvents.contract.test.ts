...
test('护士视频使用欢迎片与双待机显式白名单且切换期间最多只有一个可见播放器', () => {
  ...
  assert.match(idleVisualSource, /loadAndPlayIdleVideo\(video\)/)
  assert.doesNotMatch(idleVisualSource, /!idleReadyRef\.current\[videoIndex\]/)
  assert.match(idleVisualSource, /video\.networkState === HTMLMediaElement\.NETWORK_EMPTY\) video\.load\(\)/)
