...
import { extractSymptomNarrative, generateSymptomSummary, inferSymptomCategory, isSemanticSymptomLocation, toSymptomLocations } from './symptomRecordLogic'
...
export function SymptomRecordFlow({ memberId, token, onBack, onClose, onConfirm, onSaved }: { memberId: string; token: string; onBack: () => void; onClose: () => void; onConfirm: SaveRecord; onSaved: (message: string) => void }) {
  ...
  const [saving, setSaving] = useState(false), [pageError, setPageError] = useState(''), [extracted, setExtracted] = useState(false)
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { const result = extractSymptomNarrative(draft.narrative); setDraft((current) => ({ ...current, keywords: result.keywords, locationText: current.locationText || result.bodyLocation || '' })); setExtracted(result.keywords.length > 0) }, 350)
  ...
  return <div className="symptom-record-page-layer"><section aria-label="记录症状" aria-modal="true" className="symptom-record-page" role="dialog">
    ...
    <div className="symptom-record-scroll">
      ...
      <section className="symptom-narrative"><div className="symptom-heading"><h2>主要症状（主述）</h2><button aria-label="语音输入症状" className={listening ? 'is-listening' : ''} onClick={startVoice} type="button"><Mic size={19} /></button></div><textarea ref={narrativeRef} aria-describedby={fieldErrors.narrative ? 'symptom-narrative-error' : undefined} aria-invalid={Boolean(fieldErrors.narrative)} aria-label="主要症状" autoFocus maxLength={1000} onBlur={() => { if (narrativeRef.current) narrativeRef.current.scrollTop = 0; if (!draft.narrative.trim()) setFieldErrors((current) => ({ ...current, narrative: '请填写主要症状' })) }} onChange={(event) => updateNarrative(event.target.value)} placeholder="描述哪里不舒服、有什么变化" value={draft.narrative} />{fieldErrors.narrative && <p className="symptom-field-error" id="symptom-narrative-error" role="alert">{fieldErrors.narrative}</p>}{extracted && draft.keywords.length > 0 && <p className="symptom-extraction-note">已从主述填写</p>}{draft.keywords.length > 0 && <div className="symptom-keywords">{draft.keywords.map((keyword) => <button aria-label={`移除${keyword}`} key={keyword} onClick={() => update('keywords', draft.keywords.filter((item) => item !== keyword))} type="button">{keyword}<X size={12} /></button>)}</div>}</section>
      <section className="symptom-location-compact"><div className="symptom-heading"><h2>症状部位</h2></div><div className="symptom-location-input"><input ref={locationRef} aria-describedby={fieldErrors.location ? 'symptom-location-error' : undefined} aria-invalid={Boolean(fieldErrors.location)} aria-label="症状部位" maxLength={120} onChange={(event) => { update('locationText', event.target.value); setFieldErrors((current) => ({ ...current, location: undefined })) }} placeholder="例如：左肘窝" value={draft.locationText} /><BodyLocationPicker buttonLabel={selectedLocation ? '修改' : '定位'} compact label="" showEmptyState={false} value={draft.locations} onChange={(locations) => { setDraft((current) => ({ ...current, locations, locationText: locations[0]?.label ?? current.locationText })); setFieldErrors((current) => ({ ...current, location: undefined })); setPageError('') }} /></div>{fieldErrors.location && <p className="symptom-field-error" id="symptom-location-error" role="alert">{fieldErrors.location}</p>}{selectedLocation && <div className="symptom-location-preview"><span aria-hidden="true" className="symptom-location-silhouette"><PersonStanding size={34} strokeWidth={1.35} /><i>1</i></span><span><strong>{selectedLocation.label} · 1号区域</strong><small>{`身体${selectedLocation.locationType === 'surface' ? '表面' : '内部'} · ${selectedLocation.view === 'back' ? '背面' : '正面'}`}</small></span></div>}</section>
      ...
      {pageError && <p className="symptom-save-error" role="alert">{pageError}</p>}<div className="symptom-record-save"><HohoButton disabled={!canSave || saving} fullWidth loading={saving} onClick={() => void save()} size="large">保存</HohoButton></div>
    </div>
