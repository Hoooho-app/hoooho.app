...
test('bowel form exposes the confirmed fields without diagnostic or low-value choices', () => {
  for (const label of ['硬小颗粒', '细小颗粒', '成团偏硬', '光滑条状', '松散软块', '糊状', '水样', '无法判断', '1–2分钟', '超过10分钟', '疑似看到', '少量', '多量', '奶瓣或食物残渣']) assert.match(source, new RegExp(label))
  assert.doesNotMatch(source, /很臭|许多屁|胃痉挛|布里斯托|正常|异常|便秘|腹泻|过敏|腹胀|肚子痛|没有特别发现/)
  assert.equal((source.match(/type="range"/g) ?? []).length, 1)
  assert.match(source, /<ScaleChoice label="分量"/)
