...
export function JournalRecorder({ mode, memberId, token, onClose, onConfirm }: {
  ...
  return <div style={{ '--journal-viewport-height': `${viewport.height}px`, '--journal-keyboard-inset': `${viewport.inset}px` } as CSSProperties}><BottomSheetSurface className="journal-recorder-sheet" open label={editing ? '记录内容' : '分类提醒板'} title={editing ? '记录到今天' : '记下新情况'} onClose={() => { if (!saving) onClose() }}
    ...
    {!editing ? <><Typography variant="caption">先记下来，不用一次性记完，想到时继续补充</Typography>{journalCategoryGroups.map((group) => <section className="journal-category-group" key={group.label} aria-label={group.label}><h3 className="hoho-text-label">{group.label}</h3><div>{group.items.map(([category, label]) => <HohoButton variant="secondary" key={category} aria-pressed={selected.includes(category)} onClick={() => setSelected((current) => current.includes(category) ? current.filter((item) => item !== category) : [...current, category])}><JournalCategoryIcon category={category} />{label}</HohoButton>)}</div></section>)}</> :
