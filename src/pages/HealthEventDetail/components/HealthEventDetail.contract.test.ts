...
const changeSheetSource = readFileSync(new URL('./HealthChangeAnnotationSheet.tsx', import.meta.url), 'utf8')
...
test('症状记录使用轻量行、详情抽屉和左滑编辑删除', () => {
  ...
  assert.match(recordSheetSource, /主要症状/)
  assert.equal(recordSheetSource.includes('<h3>症状标签</h3>'), false)
  assert.match(recordSheetSource, /symptom-record-detail-actions/)
  assert.match(recordSheetSource, /删除这条记录/)
  assert.match(recordSheetSource, /症状信息/)
  assert.match(recordSheetSource, /关联记录/)
...
test('详情抽屉打开时移除页面快捷记录并保留安全区、滚动锁与可滚动正文', () => {
  ...
  assert.match(timelineSource, /onDetailOpenChange\?\.\(Boolean\(selection \|\| changeSelection\)\)/)
...
test('变化标签跟随记录、支持多标签换行且无标签时不占空白', () => {
  assert.match(timelineSource, /annotations\.length > 0 && <div[^>]*health-change-tags/)
  assert.match(timelineSource, /annotations\.map/)
  assert.match(timelineSource, /healthChangeTypeLabel\[annotation\.changeType\]/)
  assert.match(stylesSource, /health-change-tags[^}]*flex-wrap/)
  assert.match(stylesSource, /health-change-tag\[data-change='worsened'\]/)
  assert.match(stylesSource, /health-change-tag\[data-change='improved'\]/)
})

test('点击变化标签在当前页纠正或删除并提供键盘与失败反馈', () => {
  assert.match(timelineSource, /onOpenAnnotation\(annotation\)/)
  assert.match(changeSheetSource, /role="radiogroup"/)
  assert.match(changeSheetSource, /role="radio"/)
  assert.match(changeSheetSource, /aria-checked=\{selected\}/)
  assert.match(changeSheetSource, /删除这个标签/)
  assert.match(timelineSource, /标签修改失败，原状态已保留/)
  assert.match(timelineSource, /标签删除失败，请稍后重试/)
})
