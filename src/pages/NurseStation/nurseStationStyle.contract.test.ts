...
test('核心记录入口为等宽双列，更多服务为可扩展四列宫格', () => {
  ...
  assert.match(source, /label="就诊情况单"/)
  assert.match(source, /nurse-more-service--unavailable[^>]*disabled/)
  assert.doesNotMatch(source, /nurse-more-services-status|其余灰色服务正在准备中|过敏出示、能不能吃和附近就医正在准备中/)
  assert.doesNotMatch(source, /setNoticeKey|nurse-more-service-notice/)
