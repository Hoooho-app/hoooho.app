...
test('顶部将人物信息、守护天数、轮播事实和真实视频收纳为单一品牌区域', () => {
  ...
  assert.match(styles, /\.nurse-station-hero\s*\{[^}]*height:\s*150px[^}]*border:\s*1px solid #dcedea[^}]*border-radius:\s*19px[^}]*background:\s*#fff/)
  assert.match(styles, /\.nurse-station-fact\s*\{[^}]*height:\s*36px[^}]*margin-top:\s*8px[^}]*overflow:\s*hidden/)
  assert.match(styles, /\.nurse-station-visual\s*\{[^}]*height:\s*128px[^}]*background:\s*#fff/)
