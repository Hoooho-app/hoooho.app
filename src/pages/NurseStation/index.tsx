  ...
  return <main className="app-shell nurse-station-page">
    ...
    <div className="nurse-station-scroll">
      ...
      <section aria-busy={listState.status === 'loading' || !stationIsCurrent} className="guardian-tasks">
        <header><div className="guardian-task-heading"><button aria-expanded={taskViewOpen} className="guardian-task-view" onClick={() => setTaskViewOpen((value) => !value)} type="button">{taskView === 'active' ? '守护任务' : '已归档任务'}<ChevronDown /></button>{member && <p title={member.name}><strong>{member.name}</strong>的任务</p>}</div>{taskViewOpen && <div className="guardian-task-view-menu"><button onClick={() => { setTaskView('active'); setTaskViewOpen(false) }} type="button">守护任务</button><button onClick={() => { setTaskView('archive'); setTaskViewOpen(false) }} type="button">已归档任务</button></div>}</header>
...
function MoreServices({ hasHealthData, loading, onMedicalPrep }: { hasHealthData: boolean; loading: boolean; onMedicalPrep: () => void }) { const services = [{ id: 'allergy-card', label: '过敏出示', icon: Languages }, { id: 'food', label: '能不能吃', icon: Utensils }, { id: 'nearby', label: '附近就医', icon: MapPin }] as const; const unavailable=loading||!hasHealthData; const statusCopy=loading?'正在读取当前人物记录；其余灰色服务正在准备中。':hasHealthData?'过敏出示、能不能吃和附近就医正在准备中。':'就医准备需先记录健康情况；其余灰色服务正在准备中。'; return <section aria-describedby="more-services-status" className="nurse-more-services"><h2>更多服务</h2><div><MedicalPrepButton aria-describedby={unavailable ? 'medical-prep-hint' : undefined} aria-label={loading?'就医准备，正在加载当前人物记录':'就医准备'} className="journal-subject-summary" disabled={unavailable} onClick={onMedicalPrep} />{services.map(({ id, label, icon: Icon }) => <button aria-describedby="more-services-status" aria-label={label + '，暂未开放'} className={'nurse-more-service nurse-more-service--unavailable nurse-more-service--' + id} disabled key={id} type="button"><span><Icon aria-hidden="true" /></span><strong>{label}</strong></button>)}</div>{unavailable && <span className="sr-only" id="medical-prep-hint">{loading?'正在加载当前人物记录':'记录健康情况后即可生成'}</span>}<p className="nurse-more-services-status" id="more-services-status">{statusCopy}</p></section> }
