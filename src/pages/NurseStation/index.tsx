import { Bell, ChevronDown, ChevronRight, ClipboardCheck, FileText, FolderHeart, FolderOpen, HeartHandshake, Languages, MapPin, NotebookPen, Pause, Pill, Play, Plus, ShieldCheck, Syringe, Thermometer, Utensils, X } from 'lucide-react'
...
function MoreServices({ hasJournal, onMedicalPrep }: { hasJournal: boolean; onMedicalPrep: () => void }) { const [noticeKey, setNoticeKey] = useState(0); const services = [{ id: 'allergy-card', label: '过敏出示', icon: Languages }, { id: 'food', label: '能不能吃', icon: Utensils }, { id: 'nearby', label: '附近就医', icon: MapPin }] as const; useEffect(() => { if (!noticeKey) return; const timeout = window.setTimeout(() => setNoticeKey(0), 1800); return () => window.clearTimeout(timeout) }, [noticeKey]); return <section className="nurse-more-services"><h2>更多服务</h2><div>{hasJournal && <MedicalPrepButton className="journal-subject-summary" onClick={onMedicalPrep} />}{services.map(({ id, label, icon: Icon }) => <button aria-label={label + '，功能即将开放'} className={'nurse-more-service nurse-more-service--unavailable nurse-more-service--' + id} key={id} onClick={() => setNoticeKey((value) => value + 1)} type="button"><span><Icon aria-hidden="true" /></span><strong>{label}</strong></button>)}</div>{noticeKey > 0 && <div aria-live="polite" className="nurse-more-service-notice" role="status">功能即将开放</div>}</section> }
