...
export function NurseStationPage() {
  ...
  const saveMedicationPlan = (plan: NonNullable<NurseStationItem['medicationPlan']>) => {
    ...
    else setStation((previous) => ({ ...previous, items: [...previous.items, { id, memberId: currentMemberId, sourceEventId: '', relatedEventIds: [], type: 'medication_reminder', status: 'active', title: plan.medicationName, sourceLabel: '用药提醒', createdAt: now, updatedAt: now, medicationPlan: plan, reminder: { at: plan.nextOccurrenceAt, paused: false } }] }))
  ...
  return <main className="app-shell nurse-station-page">
    ...
    {savedItemId && <p aria-live="polite" className="nurse-station-save-notice" role="status">用药提醒已保存</p>}{reminderFlow && member && <MedicationReminderFlow initial={reminderFlow===true?undefined:reminderFlow} memberName={member.name} onClose={()=>setReminderFlow(null)} onSave={saveMedicationPlan} recentPlans={station.items.flatMap(item=>item.medicationPlan?[item.medicationPlan]:[])}/>}<span hidden />
...
function TaskCard({ item, onOpen }: { item: NurseStationItem; onOpen: () => void }) { const plan=item.medicationPlan; const permission=typeof Notification==='undefined'?'unsupported':Notification.permission; const status=plan?effectiveStatus(item.status,plan,permission):item.status; const label=({active:'进行中',pending_confirmation:'待提醒',due:'待处理',snoozed:'已推迟',skipped_current:'本次跳过',paused:'已暂停',completed:'已结束',ended:'已结束',notification_disabled:'通知未开启'} as Record<string,string>)[status]??taskStatus(item); return <button className="guardian-task-card" data-status={status} data-task-id={item.id} onClick={onOpen} type="button"><span className="guardian-task-icon">{item.type === 'medication_reminder' ? <Pill /> : <Thermometer />}</span><span className="guardian-task-copy"><span><strong>{plan?.medicationName??taskTitle(item)}</strong><em>{label}</em></span><small>{plan ? (status==='snoozed' ? `本次已推迟至${formatOccurrence(plan.nextOccurrenceAt)} · 原计划${formatOccurrence(plan.originalOccurrenceAt??plan.nextOccurrenceAt)}` : `${formatOccurrence(plan.nextOccurrenceAt)} · ${planLabel(plan)}`) : taskNextStep(item)}</small>{plan&&<span className="guardian-plan-tags"><i>每次{plan.amount}{plan.unit}</i><i>{routeLabel(plan.route)}</i><i>{plan.longTerm?'长期':plan.durationDays?`持续${plan.durationDays}天`:plan.endDate?`至${plan.endDate}`:'单次'}</i></span>}</span></button> }
...
function MoreServices({ hasHealthData, loading, onMedicalPrep }: { hasHealthData: boolean; loading: boolean; onMedicalPrep: () => void }) { const [noticeKey, setNoticeKey] = useState(0); const services = [{ id: 'allergy-card', label: '过敏出示', icon: Languages }, { id: 'food', label: '能不能吃', icon: Utensils }, { id: 'nearby', label: '附近就医', icon: MapPin }] as const; useEffect(() => { if (!noticeKey) return; const timeout = window.setTimeout(() => setNoticeKey(0), 1800); return () => window.clearTimeout(timeout) }, [noticeKey]); const unavailable=loading||!hasHealthData; return <section className="nurse-more-services"><h2>更多服务</h2><div><MedicalPrepButton aria-describedby={unavailable ? 'medical-prep-hint' : undefined} aria-label={loading?'就医准备，正在加载当前人物记录':'就医准备'} className="journal-subject-summary" disabled={unavailable} onClick={onMedicalPrep} />{services.map(({ id, label, icon: Icon }) => <button aria-label={label + '，功能即将开放'} className={'nurse-more-service nurse-more-service--unavailable nurse-more-service--' + id} key={id} onClick={() => setNoticeKey((value) => value + 1)} type="button"><span><Icon aria-hidden="true" /></span><strong>{label}</strong></button>)}</div>{unavailable && <span className="sr-only" id="medical-prep-hint">{loading?'正在加载当前人物记录':'记录健康情况后即可生成'}</span>}{noticeKey > 0 && <div aria-live="polite" className="nurse-more-service-notice" role="status">功能即将开放</div>}</section> }
