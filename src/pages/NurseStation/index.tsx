...
export function NurseStationPage() {
  ...
  useEffect(() => { setStation(readNurseStationState(identityId, currentMemberId)); setSelected(null) }, [identityId, currentMemberId])
  ...
  return <main className="app-shell nurse-station-page">
    ...
    <div className="nurse-station-scroll">
      {member && <button className="nurse-station-member" onClick={() => navigate('/family')} type="button"><Avatar name={member.name} src={member.avatar} size="sm" /><span><strong>{member.name}</strong><small>当前记录对象</small><em>{genderLabels[member.gender ?? '']} · {member.age}</em></span><ChevronRight size={19} /></button>}
