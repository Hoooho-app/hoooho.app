...
const symptom=/发热|发烧|体温|咳嗽|疼|痛|红疹|皮疹|瘙痒|腹泻|呕吐|喘|鼻塞|流涕|肿|不适|过敏反应/
const medication=/用药|服药|服用|药物|药片|滴剂|混悬液|胶囊|颗粒|喷雾|软膏|片$/
const day=86400000

export const isMedicationOnlyConcern = (title:string) => medication.test(title.trim()) && !symptom.test(title.trim())

export function buildVisitConcernCandidates(events:HealthEventApiDto[],now=new Date()):VisitConcernCandidate[]{
  const sorted=events.filter(event=>active(event)&&!isMedicationOnlyConcern(event.title)).sort((a,b)=>Date.parse(b.updatedAt)-Date.parse(a.updatedAt));const groups:HealthEventApiDto[][]=[]
