...
function ProcessingPanel({ item, token, open, onClose, onChange }: { item: OpsFeedbackRecord; token: string; open: boolean; onClose: () => void; onChange: (item: OpsFeedbackRecord) => void }) {
  ...
  const save = async (event: FormEvent) => {
    ...
    setError('')
    try {
      const next = await updateOpsFeedback(token, item.id, { status, priority, handledVersion: handledVersion.trim(), noActionReason: reason, mergedIntoId: mergeTarget, officialReply: status === 'not_planned' && reason !== (item.noActionReason ?? '').trim() ? reason : undefined })
      onChange(next)
    } catch (cause) {
  ...
  return <aside className="ops-feedback-processing" data-open={open} aria-label="处理面板">
    ...
    <form onSubmit={save}>
      ...
      <section><h3>处理信息</h3><label className="ops-feedback-field"><span>内部优先级</span><select value={priority} onChange={(event) => setPriority(event.target.value as FeedbackPriority)}>{Object.entries(priorityLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label className="ops-feedback-field"><span>关联上线版本</span><input value={handledVersion} onChange={(event) => setHandledVersion(event.target.value)} placeholder="例如 1.8.0" /></label>{status === 'merged' && <label className="ops-feedback-field"><span>合并到反馈 ID</span><input value={mergedIntoId} onChange={(event) => setMergedIntoId(event.target.value)} /></label>}{status === 'not_planned' && <label className="ops-feedback-field"><span>不采纳原因 · 用户可见</span><textarea value={noActionReason} onChange={(event) => setNoActionReason(event.target.value)} placeholder="说明当前无法采纳的原因" /></label>}</section>
