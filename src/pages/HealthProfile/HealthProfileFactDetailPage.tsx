  ...
  return <main className="app-shell health-profile-detail-shell">
    ...
    <div className="page-content health-profile-page-content">
      ...
      {fact && !editing && <>
        ...
        <section className="mt-7 grid gap-3"><div className="flex items-center justify-between"><Typography variant="sectionTitle">相关来源</Typography><Typography variant="caption">{fact.sources.length} 条随记内容</Typography></div><div className="health-fact-source-list">{fact.sources.map((source) => <button className="health-fact-source-row" key={`${source.organizationId}:${source.sourceFactId}`} onClick={() => navigate(`/health-events/${source.eventId}?recordId=${encodeURIComponent(source.recordId)}`)} type="button"><Link2 size={18} /><span className="min-w-0 flex-1"><strong>{formatHealthFactDate(source.recordOccurredAt)} · {source.eventTitle}</strong><small>“{source.originalText}”</small></span><ExternalLink size={17} /></button>)}</div><Typography variant="caption">来源由原始健康随记保留，不能在这里修改或删除。</Typography></section>
      </>}
      {fact && editing && <form className="health-fact-form" id="health-fact-edit-form" onSubmit={(event) => { event.preventDefault(); void save() }}>
        ...
        <section className="health-fact-provenance"><Link2 size={18} /><div><strong>来源不可修改</strong><p>已永久保留 {fact.sources.length} 条原始随记内容。</p></div></section>
