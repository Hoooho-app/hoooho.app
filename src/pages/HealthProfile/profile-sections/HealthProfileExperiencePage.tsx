...
import {
  ...
  getCurrentProfileValue,
  normalizeLegacyProfile,
  normalizeProfileValues,
  saveCurrentProfile,
  shouldShowProfileField,
  sortProfileRecords,
} from "../../../features/health-profile/utils/profileSectionExperience";
import {
  AddRecordButton,
  EditorHeader,
  FileField,
...
function loadRecords(storageKey: string, sectionId: string) {
  try {
    ...
    return parsed.map((record) =>
      normalizeLegacyProfile(sectionId, normalizeProfileValues(record)),
    );
  } catch {
    return [];
  }
...
function ProfileField({
  ...
  if (field.kind === "attachment")
    return <FileField field={field} onUnavailable={onUnavailable} />;
  if (field.kind === "textarea")
    return (
      <label className="hoho-field">
        <span className="hoho-text-label">{field.label}</span>
        <textarea
  ...
  return (
    <label className="hoho-field min-w-0">
      ...
      <span className="relative min-w-0">
        <input
          className="hoho-input w-full min-w-0 pr-14"
          onChange={(event) => onChange(event.target.value)}
          placeholder={field.placeholder}
          type={field.kind}
          value={String(value ?? "")}
...
function ProfileHero({
  ...
  definition: ProfileExperienceDefinition;
  ...
  if (definition.id === "sleep")
    return (
      <HealthCard className="grid gap-3">
        <Typography variant="sectionTitle">典型睡眠</Typography>
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 text-center">
          <strong>{String(values.bedtime || "—")}</strong>
          <span className="h-px bg-primary/40" />
          <strong>{String(values.wakeTime || "—")}</strong>
        </div>
  ...
  if (definition.id === "menstrual")
    return (
      <HealthCard>
        <Typography variant="sectionTitle">典型周期</Typography>
        <p className="mt-2 text-sm text-text-secondary">
          {summary || "填写周期和经期时长后形成长期周期画像"}
        </p>
      </HealthCard>
    );
  if (definition.id === "birth" && summary)
    return (
      <p className="rounded-control bg-primary-soft px-3 py-2 text-sm text-primary">
        {summary}
      </p>
    );
  if (definition.id === "growth" && records.length > 0) {
    const latest = sortProfileRecords(records)[0];
    return (
      <HealthCard>
        <Typography variant="sectionTitle">当前</Typography>
        <div className="mt-3 grid grid-cols-2 gap-3">
          <div>
            <strong className="text-xl">
              {String(latest.height || "—")} cm
            </strong>
            <p className="text-xs text-text-secondary">最近身高</p>
          </div>
          <div>
            <strong className="text-xl">
              {String(latest.weight || "—")} kg
            </strong>
            <p className="text-xs text-text-secondary">最近体重</p>
          </div>
        </div>
      </HealthCard>
    );
  }
  if (definition.id === "smoking" && summary)
    return <p className="text-sm font-medium text-primary">{summary}</p>;
  return null;
}

function fieldGroups(fields: ProfileExperienceField[], values: ProfileValues) {
  const visible = fields.filter((field) =>
    shouldShowProfileField(field, values),
  );
  return visible.reduce<
    Array<{ title: string; fields: ProfileExperienceField[] }>
  >((groups, field) => {
...
export function HealthProfileExperiencePage({
  definition,
  member,
  storageKey,
  title,
  ...
  const [values, setValues] = useState<ProfileValues>(() =>
    definition.repeatable
      ? {}
      : getCurrentProfileValue(loadRecords(storageKey, definition.id)),
  );
  ...
  const groups = useMemo(
    () => fieldGroups(definition.fields, values),
    [definition.fields, values],
  );
  const matrixFields =
    definition.id === "mobility"
      ? definition.fields.filter((field) => field.group === "日常生活能力")
      : [];
  ...
  const openNew = () => {
    ...
    window.setTimeout(
      () =>
        document
          .getElementById("profile-experience-editor")
          ?.scrollIntoView({ behavior: "smooth", block: "start" }),
      0,
    );
  };
  const edit = (record: ProfileValues, index: number) => {
    setValues(record);
    ...
    <>
      <div className="page-content health-profile-page-content">
        ...
        <ProfileHero
          ...
          values={values}
        />
        {definition.id === "examination" && (
          <UploadReportAction
            onUnavailable={() =>
              setStatus("附件功能暂未开放，可先手动添加报告摘要")
            }
          />
        )}
        {definition.repeatable && (
          <ProfileRecordList
            emptyDescription={
              definition.emptyDescription ?? "需要时再添加即可。"
            }
            emptyTitle={definition.emptyTitle ?? "暂无记录"}
        ...
        {definition.mode === "timeline" && (
          <ProfileTimeline records={displayedRecords} />
        )}
        {definition.repeatable && !editorOpen && (
          <AddRecordButton onClick={openNew}>
            {definition.addLabel ?? "添加一条记录"}
          </AddRecordButton>
        )}
        {editorOpen && (
          <form
            className="grid gap-6 rounded-card border bg-surface p-4"
            id="health-profile-experience-form"
            onSubmit={submit}
          >
            <div id="profile-experience-editor">
              <EditorHeader
                onClose={definition.repeatable ? closeEditor : undefined}
                title={
                  definition.repeatable
                    ? editingIndex == null
                      ? (definition.addLabel ?? "新增记录")
                      : "编辑记录"
                    : "所有字段均可留空"
                }
              />
            ...
            {groups.map((group) => {
              const fields = group.fields.filter(
                (field) => !matrixFields.includes(field),
              );
              if (group.title === "日常生活能力" && matrixFields.length > 0)
                return (
                  <ProfileSectionLead key={group.title} title={group.title}>
                    <ProfileStatusMatrix
                      fields={matrixFields}
                      onChange={change}
                      values={values}
      ...
      {editorOpen && (
        <HealthProfileActionBar split={definition.repeatable}>
            {definition.repeatable && (
              <HohoButton fullWidth onClick={closeEditor} variant="secondary">
                取消
              </HohoButton>
            )}
            <HohoButton fullWidth form="health-profile-experience-form" type="submit">
              {definition.repeatable
                ? editingIndex == null
                  ? "保存记录"
                  : "保存修改"
