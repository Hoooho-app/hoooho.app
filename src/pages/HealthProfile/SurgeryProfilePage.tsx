...
import { BodyLocationPicker, MemberIdentityCard, type BodyLocationGroup } from '../../components/health'
...
const surgeryLocationGroups: BodyLocationGroup[] = [
  { label: '头颈与躯干', view: 'front', options: ['头部', '颈部', '胸部', '腹部', '骨盆'] },
  { label: '上肢', view: 'front', options: ['左肩', '右肩', '左臂', '右臂', '左手', '右手'] },
  { label: '下肢', view: 'front', options: ['左髋', '右髋', '左膝', '右膝', '左腿', '右腿', '左踝', '右踝', '左足', '右足'] },
  { label: '背面', view: 'back', options: ['后脑', '颈后', '上背', '下背', '臀部'] },
  { label: '内部器官', view: 'internal', options: ['心脏', '肺', '肝脏', '胆囊', '胃', '肠道', '左肾', '右肾', '子宫', '其他内部器官'] }
]
...
function SurgeryFields({ record, onChange }: { record: SurgeryProfileRecord; onChange: (changes: Partial<SurgeryProfileRecord>) => void }) {
  return <div className="grid gap-5">
    ...
    <BodyLocationPicker groups={surgeryLocationGroups} label="手术部位（可多选）" onChange={(locations) => onChange({ locations })} values={record.locations} />
