...
function SurgeryFields({
  ...
  return (
    <div className="grid gap-5">
      ...
      <label className="hoho-field">
        <span className="hoho-text-label">手术原因 / 当时的问题</span>
        <textarea
      ...
      <SingleChoice
        label="术后情况"
        options={["恢复良好", "仍有一些影响", "有长期影响"]}
        value={record.recovery}
        onChange={(recovery) => onChange({ recovery })}
      />
      <label className="hoho-field">
        <span className="hoho-text-label">目前还有哪些影响（可选）</span>
        <textarea
          className="hoho-textarea"
          rows={3}
          value={record.remainingImpact}
          onChange={(event) =>
            onChange({ remainingImpact: event.target.value })
          }
        />
      </label>
      <SingleChoice
        label="是否有植入物 / 医疗器械"
        options={["有", "无"]}
        value={record.hasImplant}
        onChange={(hasImplant) =>
          onChange({
            hasImplant,
            ...(hasImplant === "无" ? { implantName: "" } : {}),
          })
        }
      />
      {record.hasImplant === "有" && (
        <label className="hoho-field">
          <span className="hoho-text-label">植入物 / 医疗器械名称（可选）</span>
          <input
            className="hoho-input"
            placeholder="例如人工关节、心脏支架"
            value={record.implantName}
            onChange={(event) => onChange({ implantName: event.target.value })}
          />
        </label>
      )}
    </div>
...
function ReportSection({
  ...
  return (
    <section className="grid gap-3 rounded-card border bg-surface p-4">
      <Typography variant="sectionTitle">手术相关资料</Typography>
      <button
...
export function SurgeryProfilePage({
  member,
  storageKey,
}: {
  member: Member;
  ...
  const initial = useState(() => {
    ...
    return normalizeSurgeryRecords(
      Array.isArray(stored) ? (stored as Record<string, unknown>[]) : [],
    );
  })[0];
  const [records, setRecords] = useState<SurgeryProfileRecord[]>(() =>
    initial.length ? initial : [emptySurgeryRecord(1)],
  );
  const [reports, setReports] = useState<SurgeryReport[]>(() =>
    normalizeSurgeryReports(loadJson(reportKey)),
  );
    ...
    files.forEach((file, index) => {
      ...
      reader.onload = () =>
        setReports((current) => [
          ...current,
          {
            id: `surgery-report-${Date.now()}-${index}`,
            name: file.name,
  ...
  return (
    <main className="app-shell min-h-dvh">
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  {!expanded && (
                    <span
                      className={`mt-1 shrink-0 rounded-pill px-2 py-1 text-xs ${record.recovery === "恢复良好" ? "bg-primary-soft text-success" : record.recovery ? "bg-warning/10 text-warning" : "bg-background text-text-secondary"}`}
                    >
                      {summary.status}
                    </span>
                  )}
                  {expanded ? (
                    ...
                    <button
                      aria-label={`展开手术 ${record.sequence}`}
      ...
      <HealthProfileActionBar>
        <HohoButton fullWidth form="surgery-profile-form" type="submit">
          保存档案
        </HohoButton>
      </HealthProfileActionBar>
    </main>
