...
export function ImportantHealthFactsPage() {
  ...
  return <main className="app-shell health-profile-detail-shell">
    ...
    <div className="page-content health-profile-page-content">
      ...
      <Typography className="mt-5" variant="caption">保存已经确认要长期保留的信息，并随时回看它来自哪条健康随记。</Typography>
      ...
      {status === 'success' && <>
        {candidates.length > 0 && <section className="health-fact-inbox mt-6" aria-label="待处理健康信息">
          <div className="flex items-start gap-3"><span className="health-fact-inbox__icon"><FileHeart size={20} /></span><div className="min-w-0 flex-1"><Typography variant="cardTitle">发现可长期保留的信息</Typography><Typography className="mt-1" variant="caption">{candidates.length} 条来自健康随记，需你确认后才会加入档案</Typography></div></div>
        ...
        <section className="mt-6 grid gap-3">
          ...
          {activeFacts.length === 0 ? <div className="health-fact-empty"><FileHeart className="text-primary" size={24} /><Typography className="mt-3" variant="cardTitle">暂无重要健康事实</Typography><Typography className="mt-1" variant="caption">健康随记中的重要信息确认后会显示在这里</Typography></div> : <div className="health-fact-list">{activeFacts.map((fact) => <button className="health-fact-row" key={fact.id} onClick={() => navigate(`/health-profile/facts/${fact.id}`)} type="button"><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><strong>{fact.title}</strong><span className="health-fact-status" data-status={fact.status}>{healthProfileFactStatusLabels[fact.status]}</span></span><small>{healthProfileFactCategoryLabels[fact.category]} · 更新于 {formatHealthFactDate(fact.updatedAt)}</small><small className="health-fact-source-count"><Link2 size={13} />{fact.sources.length} 条来源</small></span><ChevronRight size={18} /></button>)}</div>}
        </section>
