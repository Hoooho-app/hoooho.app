...
test('孩子生日、照片、保存和删除交互有完整状态保护', () => {
  ...
  assert.match(pageSource, /readMemberProfileEditOrigin/)
  assert.match(pageSource, /makeMemberProfileRestoreState/)
  assert.match(pageSource, /leaveEditor\(true\)/)
  assert.match(pageSource, /leaveEditor\(false\)/)
  assert.doesNotMatch(pageSource, /navigate\('\/family'/)
