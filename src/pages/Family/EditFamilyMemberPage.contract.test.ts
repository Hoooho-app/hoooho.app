...
test('孩子编辑页只呈现冻结的信息架构', () => {
  ...
  assert.match(pageSource, /国籍/)
  assert.match(pageSource, /请选择国籍/)
  assert.match(pageSource, /NATIONALITY_CODES/)
...
test('孩子编辑页使用真实数据、统一草稿和服务端回读确认', () => {
  ...
  assert.match(pageSource, /member\.nationality/)
  assert.match(pageSource, /nationality: draft\.nationality \|\| null/)
