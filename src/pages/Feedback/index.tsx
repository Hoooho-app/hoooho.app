...
export function FeedbackSubmittedPage() {
  ...
  return <main className="app-shell feedback-page pb-0"><WebPageHeader title="反馈意见" fallback="/feedback/mine"/><section className="feedback-success"><HealthTrace className="feedback-success__trace"/><h2>谢谢，你让 Hoooho 更好了一点</h2>{data?.id ? <><p>我们已经收到，会在这里更新处理进度。</p><small>反馈编号：{data.id}</small><Button fullWidth type="button" onClick={() => navigate(`/feedback/${data.id}`, { replace: true })}>查看处理进度</Button></> : <p>无法确认本次反馈已经保存，请返回后重新提交。</p>}<button type="button" onClick={() => navigate(source.path, { replace: true })}>继续使用 Hoooho</button></section></main>
}
