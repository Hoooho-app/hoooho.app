  ...
  return (
    <main className="app-shell pb-0">
      ...
      <form className="space-y-3 px-4 py-4" onSubmit={submit}>
        <section className="rounded-[16px] bg-surface p-4 shadow-card">
          <h2 className="text-sm font-medium">我要反馈</h2>
          <p className="mt-1.5 text-xs leading-6 text-text-secondary">告诉我们你的问题或建议，帮助我们持续改进。</p>
        </section>
        <textarea className="h-[150px] w-full resize-none rounded-[16px] bg-surface p-4 text-[13px] outline-none shadow-card placeholder:text-text-secondary focus:ring-2 focus:ring-primary/20" placeholder="请描述你遇到的问题或建议…" value={content} onChange={(event) => { setContent(event.target.value); setError('') }} />
        <label className="flex min-h-[91px] cursor-pointer items-center gap-3 rounded-[16px] bg-surface p-4 shadow-card">
  ...
  return (
    <main className="app-shell pb-0">
      ...
      <div className="space-y-3 px-4 py-4">
        <section className="rounded-[16px] bg-surface p-4 shadow-card"><h2 className="text-sm font-medium">反馈已提交</h2><p className="mt-1.5 text-xs leading-6 text-text-secondary">我们会尽快处理，你可以在“我的反馈”中查看进度。</p></section>
        <h2 className="pt-1 text-base font-medium">我的反馈</h2>
        {feedbacks.map(([title, date, status, color]) => (
          <article key={title} className="rounded-[16px] bg-surface p-4 shadow-card"><h3 className="text-sm font-medium">{title}</h3><p className="mt-1.5 text-xs text-text-secondary">{date}</p><p className={`mt-3 text-xs font-medium ${color}`}>{status}</p></article>
        ))}
      </div>
    </main>
  )
