...
export const guideSectionLabels: Record<GuideSectionId, { description: string; title: string }> = {
  record: { title: '记录健康问题', description: '把刚刚发生的情况，准确地留在对应事件里。' },
...
export const guideTutorials: GuideTutorial[] = [
  {
    ...
    keywords: ['一句话', '新建事件', '快捷记录', '发热', '体温', '第一次', '孩子'],
    steps: ['开始一次健康事件', '用一句话写下发生了什么', '核对时间与描述后保存'],
    result: '建立一条发热事件，第一条记录和体温变化会进入时间线。', actionLabel: '现在去记录', actionTo: '/health-events'
  },
  {
    id: 'continue-event', title: '体温又变了，怎么接着补充？', context: '同一件事还在发展，不需要重新建立事件。',
    section: 'observe', filterIds: ['first', 'record'], core: true, media: coreMedia('continue-event'),
    keywords: ['继续记录', '补充', '新增记录', '症状变化', '体温变化', '时间线'],
    steps: ['打开已有健康事件', '点快捷记录并描述新变化', '检查整理结果后确认'],
    result: '新增内容出现在原事件时间线中，体温趋势也会同步更新。', actionLabel: '查看健康事件', actionTo: '/health-events'
  },
  {
    id: 'prepare-doctor', title: '准备问医生，怎么带走病情资料？', context: '不想在问诊时从头回忆，可以先使用事件中已经整理好的内容。',
    section: 'visit', filterIds: ['first', 'visit'], core: true, media: coreMedia('prepare-doctor'),
    keywords: ['医生', '就诊', '在线问诊', '病情描述', '复制', '资料', '下一步'],
    steps: ['打开事件详情的“下一步”', '进入在线问诊资料', '复制病情描述或全部内容'],
    result: '可以把已整理的病情描述复制到当前问诊渠道继续使用。', actionLabel: '去选择一个事件', actionTo: '/health-events'
  },
  {
    ...
    steps: ['开始一次健康事件', '把开始时间改为实际发生时间', '填写情况并保存'], result: '记录会按实际发生时间进入健康事件。',
  ...
  },
  {
    ...
    actionLabel: '去打开一个事件', actionTo: '/health-events'
  },
  {
    id: 'upload-first-photo', title: '有检查单或药盒照片，怎么一起记录？', context: '第一次记录事件时，可以把相关图片和文字放在同一条记录里。',
    section: 'record', filterIds: ['record'], keywords: ['图片', '照片', '检查单', '药盒', '附件', '上传'],
    steps: ['开始一次健康事件', '在附件补充中选择图片', '核对文字和时间后保存'], result: '图片会归到这条健康事件记录中。',
    actionLabel: '开始一次记录', actionTo: '/health-events'
  },
  {
    id: 'read-timeline', title: '事情发展了几天，怎么从头看完整过程？', context: '同一个事件里的每次补充，会按发生时间集中展示。',
    section: 'observe', filterIds: ['record'], keywords: ['时间线', '完整过程', '顺序', '记录详情', '发生了什么', '措施'],
    steps: ['打开一个健康事件', '查看按日期分组的时间线', '展开单条记录查看细节'], result: '症状、体温和已经采取的措施会沿时间顺序呈现。',
    actionLabel: '查看健康事件', actionTo: '/health-events'
  },
  {
    id: 'temperature-trend', title: '体温忽高忽低，怎么查看变化？', context: '事件中记录过多个体温后，会出现体温趋势。',
    section: 'observe', filterIds: ['record'], keywords: ['体温', '趋势', '变化', '高热', '低热', '测量'],
    steps: ['连续记录至少两次体温', '回到事件详情', '查看时间点和温度变化'], result: '每个温度和测量时间会一起显示，趋势只作记录参考。',
    actionLabel: '去记录体温', actionTo: '/health-events'
  },
  {
    id: 'prepare-online-consultation', title: '在线问诊前，怎么减少重复输入？', context: '事件已有记录时，可以从“下一步”进入在线问诊资料。',
    section: 'visit', filterIds: ['visit'], keywords: ['在线问诊', '复制全部', '复制病情', '资料', '医生'],
    steps: ['打开事件详情', '点“下一步”后进入在线问诊', '按需要复制单项或全部资料'], result: '已有病情描述可以直接带到问诊平台继续使用。',
    actionLabel: '去选择一个事件', actionTo: '/health-events'
  },
  {
    id: 'add-family-member', title: '想替孩子记录，先怎么添加家人？', context: '每位家人的健康事件和档案彼此分开。',
  ...
  {
    id: 'switch-family-member', title: '记录前，怎么确认写给了正确的人？', context: '健康事件页会显示当前家人，记录前可以先切换人物。',
    section: 'family', filterIds: ['family'], keywords: ['切换人物', '记录对象', '当前家人', '替家人记录'],
    steps: ['在健康事件页查看当前家人', '点“切换人物”', '选择要记录的家庭成员'], result: '之后新建的健康事件会归到选中的人物。',
    actionLabel: '管理家人', actionTo: '/family'
  },
  {
    id: 'record-allergy', title: '有明确过敏，应该记在哪里？', context: '过敏属于长期健康背景，不必混在某一次事件里。',
