...
const female: HealthProfileType[] = ['teen', 'adult-female', 'elder-female']
...
export const healthProfileSections: HealthProfileSectionConfig[] = [
  ...
  { id:'family-history', title:'家族遗传与重要病史', description:'家族成员的重要疾病情况', guidance:'可记录父母、兄弟姐妹及祖辈的重要疾病情况。', category:'core', icon:'family', activeFor:all, repeatable:true, fields:[field('relationship','家庭成员关系'),field('disease','疾病 / 健康问题'),field('age','大概发病年龄','number',{unit:'岁'}),field('similar','是否有其他亲属也有类似情况','select',{options:yn}),note] },
  ...
  { id:'growth', title:'生长发育', description:'身高、体重、头围与发育变化', guidance:'按需要补充阶段性生长记录。', category:'child', icon:'activity', activeFor:children, repeatable:true, fields:[field('date','日期','date'),field('height','身高 / 身长','number',{unit:'cm'}),field('weight','体重','number',{unit:'kg'}),field('headCircumference','头围','number',{unit:'cm'}),field('note','发育情况备注','textarea')] },
  ...
  { id:'menstrual', title:'月经健康', description:'长期周期、经量与不适情况', guidance:'记录长期规律和需要关注的变化即可。', category:'female', icon:'calendar', activeFor:female, fields:[field('menarche','初潮时间','date'),field('regular','周期是否规律','select',{options:yn}),field('cycle','平均周期','number',{unit:'天'}),field('duration','经期时长','number',{unit:'天'}),field('flow','经量情况','select',{options:['较少','一般','较多']}),field('pain','是否痛经','select',{options:yn}),field('abnormal','其他长期异常情况','textarea')] },
  { id:'pregnancy', title:'孕产健康', description:'妊娠与分娩的重要经历', guidance:'每次孕产经历可单独记录。', category:'female', icon:'calendar', activeFor:['adult-female','elder-female'], repeatable:true, fields:[field('date','时间','date'),field('result','妊娠结果'),field('weeks','孕周','number',{unit:'周'}),field('delivery','分娩方式'),field('situation','重要情况','textarea'),field('hospital','医院'),attachment] },
