...
const actionSheet = read('../pages/HealthEventDetail/components/ActionSheet.tsx')
...
test('旧消息和通知地址统一安全重定向到健康事件首页', () => {
  for (const path of ['/messages/*', '/message/*', '/notifications/*', '/notification/*']) {
    assert.match(router, new RegExp(`path: '${path.replaceAll('/', '\\/').replace('*', '\\*')}'[^}]+<Navigate to="\\/health-events" replace \\/>`))
  }
