...
export class FamilyMemberRepository {
  ...
  async create(input, now = new Date()) {
    const member = {
      ...
      premature: input.premature ?? null,
      gestationalWeeks: input.gestationalWeeks ?? null,
      birthWeightKg: input.birthWeightKg ?? null,
      birthLengthCm: input.birthLengthCm ?? null,
      birthHeadCircumferenceCm: input.birthHeadCircumferenceCm ?? null,
      concernFocus: Array.isArray(input.concernFocus) ? input.concernFocus : [],
      recordingPausedAt: input.recordingPausedAt ?? null,
