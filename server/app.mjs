...
async function handleOps(request, response, pathname) {
  ...
  const access = assertOpsAccess(payload, { ownerEmail: authConfig.opsOwnerEmail })
  ...
  else if (pathname === '/api/ops/resources' && request.method === 'GET') sendJson(response, 200, { ...await ops.list(), accessMode: access.mode })
