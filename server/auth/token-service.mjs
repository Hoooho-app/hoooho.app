...
export class TokenService {
  ...
  create(user, now = Date.now()) {
    ...
    const payload = encode({ sub: user.id, ...(user.email ? { email: user.email } : {}), ...(user.phone ? { phone: user.phone } : {}), ...(user.guest ? { guest: true } : {}), ...(user.purpose ? { purpose: user.purpose } : {}), iat: now, exp: now + this.ttlMs })
