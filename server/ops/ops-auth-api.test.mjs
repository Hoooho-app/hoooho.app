...
test('all Operations APIs share the same owner-only authorization', async () => {
  ...
  try {
    ...
    const protectedRequests = [
      ['/api/ops/session', 'GET'], ['/api/ops/sources', 'GET'], ['/api/ops/sources', 'POST', { name: 'Test', platformUrl: 'https://example.com/billing', method: 'manual-screenshot', frequency: 'manual' }],
      ['/api/ops/sources/railway', 'PATCH', { notes: 'test' }], ['/api/ops/refresh', 'POST'], ['/api/ops/feedback', 'GET'],
    ...
    assert.equal((await json(`${base}/api/ops/sources`, 'GET', expired)).status, 401)
    assert.equal((await json(`${base}/api/ops/sources`, 'GET', `${ownerSession.token}tampered`)).status, 401)
...
test('Operations API fails closed when the owner setting is absent', async () => {
  ...
  try {
    ...
    const address = server.httpServer?.address()
    assert.ok(address && typeof address === 'object')
    const response = await json(`http://127.0.0.1:${address.port}/api/ops/sources`, 'GET', token)
