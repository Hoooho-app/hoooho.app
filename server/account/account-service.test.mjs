...
test('stable guest identity merges server records once', async () => {
  ...
  try {
    ...
    const member = await members.create({ accountId: first.user.id, name: '孩子', relationship: 'child' })
    const routinePreferences = new JsonStore(path.join(context.dataDirectory, 'routine-preferences.json'), { preferences: [] })
    await routinePreferences.update((data) => ({ ...data, preferences: [{ id: 'routine-preference-1', accountId: first.user.id, memberId: member.id, status: 'enabled' }] }))
    ...
    assert.equal((await routinePreferences.read()).preferences[0].accountId, login.user.id)
