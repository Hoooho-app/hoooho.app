...
test('成长记录按账户和人物隔离，同日 upsert 且支持增删改', async () => {
  ...
  try {
    ...
    const same = await service.upsert('a1', { memberId: child.id, measuredAt: '2026-01-01', measurementType: 'length', heightCm: 70, weightKg: 8, standardId: 'who-2006' }, new Date('2026-02-01'))
    assert.equal(same.id, first.id); assert.equal((await service.list('a1', child.id)).length, 1); assert.equal(same.weightKg, 8)
