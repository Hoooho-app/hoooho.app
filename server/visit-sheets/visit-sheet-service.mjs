...
export class VisitSheetService {
  ...
  async get(accountId, memberId) {
    ...
    if (report) {
      ...
      if (report.sources.some((s) => !allowed.has(s.id)))
        return {
          report: null,
  ...
  async save(accountId, memberId, request, now = new Date()) {
    return withAccountLock(accountId, async () => {
      ...
      if (previous && input.warnings.length)
        throw failure(
          '部分资料未能读取，这次更新没有完成，原报告仍保留。请重试。',
          503,
        )
      const report = {
        ...buildVisitSheet(input, { focus, notes, question }, now),
      ...
      report.changes = [
        ...(previous?.changes ?? []).filter(
          (c) => c.sourceId === '家长报告编辑' || available.has(c.sourceId),
        ),
        ...report.sources
          .filter((s) => old.has(s.id) && old.get(s.id).text !== s.text)
          .map((s) => ({
            sourceId: s.id,
      ...
      if (
        ...
        report.changes.push({
          ...
          after: JSON.stringify({ focus, question, notes }),
          at: now.toISOString(),
        })
