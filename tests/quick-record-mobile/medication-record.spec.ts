...
async function prepare(page: Page) {
  ...
  await page.getByPlaceholder('给自己起个昵称').fill('用药测试'); await page.getByPlaceholder('设置一个密码').fill('12345678'); await page.getByRole('button', { name: '注册并进入' }).click(); await page.getByRole('button', { name: '进入 Hoooho' }).click()
