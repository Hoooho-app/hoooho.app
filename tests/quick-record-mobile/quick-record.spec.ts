...
test('语音权限、单实例、计时、结束、重新说和保存保持视频节点', async ({ page }) => {
  ...
  const savedVideo = page.locator('video[data-video-phase="save_success"]')
  await expect(savedVideo).toHaveAttribute('data-active', 'true')
  await expect(page.locator('video[data-video-phase="idle1"]')).toHaveAttribute('data-active', 'true', { timeout: 6000 })
  await expect(savedVideo).toHaveAttribute('data-active', 'false')
