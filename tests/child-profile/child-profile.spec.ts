...
test('添加家庭成员使用空白孩子资料表单并一次保存头像和记录者', async ({ page, request }, testInfo) => {
  ...
  try {
    ...
    await expect(page.getByText('先填写信息', { exact: true })).toBeVisible()
    await expect(page.getByRole('button', { name: '换一个' })).toHaveCount(0)
    await expect(page.locator('section[aria-label="卡通形象"] img[src*="/avatars/children/"]')).toHaveCount(0)
    ...
    await expect(page.getByText('先填写信息', { exact: true })).toHaveCount(0)
    await expect(page.getByRole('button', { name: '换一个' })).toBeVisible()
    await expect(page.locator('section[aria-label="卡通形象"] img[src*="/avatars/children/"]')).toBeVisible()
