...
test('用药卡片到点记录、逐次撤回、左滑归档和删除确认均持久化', async ({ browser, page }) => {
  ...
  await expect(dialog).toContainText('停止后续提醒，保留已有服用记录。')
  ...
  await page.mouse.move(box!.x + 40, box!.y + 35); await page.mouse.down(); await page.mouse.move(box!.x + box!.width - 10, box!.y + 35, { steps: 8 }); await page.mouse.up()
  ...
  await page.locator('.guardian-task-view').click(); await page.getByRole('button', { name: '已归档任务', exact: true }).click()
  await expect(page.locator('.medication-course-card').filter({ hasText: '短疗程用药' })).toContainText('已归档')
})
