...
async function currentMemberId(page:Page) {
  const token=pageTokens.get(page)??''
  const response=await page.request.get('/api/members',{headers:{Authorization:`Bearer ${token}`}})
  expect(response.ok()).toBe(true)
  return ((await response.json()) as Array<{id:string}>)[0].id
}

async function startDesensitization(page:Page,input:string) {
  const token=pageTokens.get(page)??'',memberId=await currentMemberId(page),headers={Authorization:`Bearer ${token}`,'Content-Type':'application/json'}
  const resolved=await page.request.post('/api/desensitization-tests/resolve',{headers,data:{memberId,input}})
  if(!resolved.ok())throw new Error(`resolve ${resolved.status()}: ${await resolved.text()}`)
  const preview=await resolved.json()
  const committed=await page.request.post('/api/desensitization-tests/commit',{headers,data:{memberId,input,selectedTaskIds:preview.selectedTaskIds,resolutionVersion:preview.resolutionVersion,operationId:crypto.randomUUID()}})
  if(!committed.ok())throw new Error(`commit ${committed.status()}: ${await committed.text()}`)
  return committed.json()
}

async function seedDietEvent(page:Page,foods:string[],index:number) {
  const token=pageTokens.get(page)??'',memberId=await currentMemberId(page),occurredAt=new Date(Date.now()-(index+1)*86_400_000).toISOString()
  const response=await page.request.post('/api/quick-records',{headers:{Authorization:`Bearer ${token}`},data:{memberId,content:foods.join('、'),occurredAt,inputChannel:'text',idempotencyKey:crypto.randomUUID(),title:'饮食记录',journal:{categories:['diet'],occurredAt,timePrecision:'exact',diet:{kind:'meal',meal:'晚餐',foods,supplementNames:[],amount:'适量',foodForm:'small-pieces',appetite:'和平时差不多',reactions:[]}}}})
  if(response.status()!==201)throw new Error(`diet ${response.status()}: ${await response.text()}`)
  return response.json()
}
...
test('排敏测试从新增到症状记录、趋势和刷新持久化形成闭环', async ({ page }) => {
  ...
  await page.getByPlaceholder('例如：面包、小麦、牛乳酪蛋白').fill('牛')
  await expect(page.getByText('这个名称需要再明确一点')).toBeVisible()
  ...
  await expect(page.getByText('原料来源', { exact: true })).toBeVisible()
  ...
  await expect(page).toHaveURL(/\/nurse-station$/)
  await expect(page.getByRole('dialog', { name: /牛肉来源.*变化与下一步/ })).toBeVisible()
  await expect(page.getByText('未记录 / 情况不清楚')).toBeVisible()
  await page.getByRole('dialog', { name: /牛肉来源.*变化与下一步/ }).getByLabel(/关闭牛肉来源.*变化与下一步/).click()

  const card = page.getByTestId('desensitization-card-牛肉来源')
  ...
  await page.reload()
  await page.getByRole('tab', { name: '排敏测试' }).click()
  await expect(page.getByTestId('desensitization-card-牛肉来源').getByText('暂停进阶', { exact: true })).toBeVisible()
  await page.getByTestId('desensitization-card-牛肉来源').getByRole('button', { name: /查看牛肉来源近7日趋势/ }).click()
  ...
  await page.getByRole('button', { name: '新增测试', exact: true }).click()
  await page.getByPlaceholder('例如：面包、小麦、牛乳酪蛋白').fill('牛奶')
  await page.getByRole('button', { name: '开始观察' }).click()
  await page.getByRole('dialog', { name: /牛乳来源.*变化与下一步/ }).getByLabel(/关闭牛乳来源.*变化与下一步/).click()
  const milkCard = page.getByTestId('desensitization-card-牛乳来源')
  ...
  await milkCard.getByRole('button', { name: /管理牛乳来源排敏测试/ }).click()
  ...
  await expect(page.getByRole('dialog', { name: /删除“牛乳来源”观察/ })).toBeVisible()
  ...
  await expect(page.getByTestId('desensitization-card-牛乳来源')).toBeVisible()

  await page.getByTestId('desensitization-card-牛乳来源').getByRole('button', { name: /管理牛乳来源排敏测试/ }).click()
  ...
  for (const viewport of [{ width: 320, height: 568 }, { width: 390, height: 844 }, { width: 430, height: 932 }, { width: 1440, height: 900 }]) {
    await page.setViewportSize(viewport)
    expect(await page.evaluate(() => document.documentElement.scrollWidth)).toBe(viewport.width)
    await expect(page.getByTestId('desensitization-card-牛肉来源')).toBeVisible()
    await page.screenshot({ path: `test-results/desensitization-card-${viewport.width}x${viewport.height}.png`, fullPage: true })
  }
})

test('跨层输入把面包作为待核实线索加入既有小麦观察且不重复建卡', async ({ page }) => {
  await registerMember(page)
  await startDesensitization(page,'小麦')
  for(let index=0;index<6;index+=1)await seedDietEvent(page,['馒头'],index)
  const token=pageTokens.get(page)??'',beforeEvents=await page.request.get('/api/events',{headers:{Authorization:`Bearer ${token}`}}),beforeCount=((await beforeEvents.json()) as unknown[]).length
  await page.goto('/nurse-station/desensitization/new')
  await page.getByPlaceholder('例如：面包、小麦、牛乳酪蛋白').fill('面包')
  await expect(page.getByRole('heading',{name:'食物与成分关联'})).toBeVisible()
  await expect(page.getByText('可能含有',{exact:true}).first()).toBeVisible()
  await expect(page.getByRole('button',{name:/小麦来源.*观察中/})).toHaveAttribute('aria-pressed','true')
  await expect(page.getByRole('button',{name:'加入「小麦来源」观察'})).toBeVisible()
  await page.screenshot({path:'test-results/desensitization-cross-layer-input-375x667.png',fullPage:true})
  await page.screenshot({path:'test-results/desensitization-bread-pending-join-375x667.png',fullPage:true})
  await page.setViewportSize({width:320,height:568})
  expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBe(320)
  await page.screenshot({path:'test-results/desensitization-cross-layer-input-320x568.png',fullPage:true})
  await page.setViewportSize({width:375,height:667})
  await page.getByRole('button',{name:'加入「小麦来源」观察'}).click()
  await page.getByRole('dialog',{name:/小麦来源.*变化与下一步/}).getByLabel(/关闭小麦来源.*变化与下一步/).click()
  await expect(page.getByTestId('desensitization-card-小麦来源')).toBeVisible()
  const list=await page.request.get(`/api/desensitization-tests?memberId=${encodeURIComponent(await currentMemberId(page))}`,{headers:{Authorization:`Bearer ${token}`}}),data=await list.json()
  expect(data.tasks).toHaveLength(1)
  expect(data.tasks[0].foodLinks.find((item:{entityId:string})=>item.entityId==='food:bread')?.evidenceStatus).toBe('pending')
  const afterEvents=await page.request.get('/api/events',{headers:{Authorization:`Bearer ${token}`}})
  expect(((await afterEvents.json()) as unknown[]).length).toBe(beforeCount)
})

test('同来源食品在页面内预览去重合并并提供食品筛选、管理关联与撤回', async ({ page }) => {
  await registerMember(page)
  await startDesensitization(page,'馒头')
  await startDesensitization(page,'小麦面条')
  await seedDietEvent(page,['馒头','小麦面条'],0)
  for(let index=1;index<=3;index+=1)await seedDietEvent(page,['馒头'],index)
  for(let index=4;index<=5;index+=1)await seedDietEvent(page,['小麦面条'],index)
  await page.goto('/nurse-station/desensitization/new')
  await page.getByPlaceholder('例如：面包、小麦、牛乳酪蛋白').fill('小麦')
  await expect(page.getByRole('button',{name:'合并为「小麦来源」'})).toBeVisible()
  await expect(page.locator('.desensitization-result-preview')).toContainText('6')
  await expect(page.locator('.desensitization-result-preview')).toContainText('去重')
  await page.screenshot({path:'test-results/desensitization-multi-merge-preview-375x667.png',fullPage:true})
  await page.setViewportSize({width:1440,height:900})
  expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBe(1440)
  await page.screenshot({path:'test-results/desensitization-multi-merge-preview-1440x900.png',fullPage:true})
  await page.setViewportSize({width:375,height:667})
  const commitResponse=page.waitForResponse((response)=>response.url().endsWith('/api/desensitization-tests/commit')&&response.request().method()==='POST')
  await page.getByRole('button',{name:'合并为「小麦来源」'}).click()
  const mergeResult=await (await commitResponse).json()
  const trend=page.getByRole('dialog',{name:/小麦来源.*变化与下一步/})
  await expect(trend.getByRole('button',{name:/食品 · 4.*馒头/})).toBeVisible()
  await expect(trend.getByRole('button',{name:/食品 · 3.*小麦面条/})).toBeVisible()
  await page.screenshot({path:'test-results/desensitization-food-filter-375x667.png',fullPage:true})
  await trend.getByLabel(/关闭小麦来源.*变化与下一步/).click()
  await expect(page.getByTestId('desensitization-card-小麦来源')).toBeVisible()
  await expect(page.getByTestId('desensitization-card-馒头')).toHaveCount(0)
  await expect(page.getByRole('button',{name:'撤销'})).toBeVisible()
  await page.screenshot({path:'test-results/desensitization-unified-card-375x667.png',fullPage:true})
  const token=pageTokens.get(page)??'',memberId=await currentMemberId(page)
  const mergedList=await page.request.get(`/api/desensitization-tests?memberId=${encodeURIComponent(memberId)}`,{headers:{Authorization:`Bearer ${token}`}}),mergedData=await mergedList.json(),redirect=mergedData.redirects[0]
  expect(redirect).toBeTruthy()
  await page.goto(`/nurse-station?desensitization=${encodeURIComponent(redirect.taskId)}`)
  const redirectedTrend=page.getByRole('dialog',{name:/小麦来源.*变化与下一步/})
  await expect(redirectedTrend.locator('.desensitization-trend-filters button').filter({hasText:redirect.filterLabel})).toHaveAttribute('aria-pressed','true')
  await redirectedTrend.getByLabel(/关闭小麦来源.*变化与下一步/).click()
  await page.getByTestId('desensitization-card-小麦来源').getByRole('button',{name:/管理小麦来源排敏测试/}).click()
  await page.getByRole('button',{name:'管理关联'}).click()
  await expect(page.getByRole('dialog',{name:'管理关联'})).toContainText('馒头')
  await expect(page.getByRole('dialog',{name:'管理关联'})).toContainText('小麦面条')
  await page.screenshot({path:'test-results/desensitization-manage-associations-375x667.png',fullPage:true})
  await page.getByRole('dialog',{name:'管理关联'}).getByLabel('关闭管理关联').click()
  await page.screenshot({path:'test-results/desensitization-merge-undo-375x667.png',fullPage:true})
  const undone=await page.request.post(`/api/desensitization-tests/operations/${encodeURIComponent(mergeResult.operationId)}/undo`,{headers:{Authorization:`Bearer ${token}`},data:{}})
  expect(undone.ok()).toBe(true)
  await page.reload()
  await page.getByRole('tab',{name:'排敏测试'}).click()
  await expect(page.getByTestId('desensitization-card-馒头')).toBeVisible()
  await expect(page.getByTestId('desensitization-card-小麦面条')).toBeVisible()
})

test('牛乳酪蛋白沿用牛乳入口并保持成分适用记录独立统计', async ({ page }) => {
  await registerMember(page)
  await startDesensitization(page,'牛乳')
  await seedDietEvent(page,['酸奶'],0)
  await seedDietEvent(page,['奶酪'],1)
  await page.goto('/nurse-station/desensitization/new')
  await page.getByPlaceholder('例如：面包、小麦、牛乳酪蛋白').fill('牛乳酪蛋白')
  await expect(page.getByRole('button',{name:'加入「牛乳来源」观察'})).toBeVisible()
  await expect(page.locator('.desensitization-result-preview')).toContainText('牛乳来源')
  await page.screenshot({path:'test-results/desensitization-component-focus-reuse-375x667.png',fullPage:true})
  await page.getByRole('button',{name:'加入「牛乳来源」观察'}).click()
  const trend=page.getByRole('dialog',{name:/牛乳来源.*变化与下一步/})
  await expect(trend.getByRole('button',{name:/成分 · 0.*牛乳酪蛋白/})).toBeVisible()
  await expect(trend.getByText(/既有来源记录不会自动改标/)).toHaveCount(0)
  await trend.getByRole('button',{name:/成分 · 0.*牛乳酪蛋白/}).click()
  await expect(trend.getByText(/既有来源记录不会自动改标/)).toBeVisible()
})

test('只有历史牛肉随记时预览并沿用三个原始事件时间', async ({ page }) => {
  await registerMember(page)
  await seedDietEvent(page,['牛肉'],0)
  await seedDietEvent(page,['清蒸牛肉'],1)
  await seedDietEvent(page,['纯牛肉泥'],2)
  await page.goto('/nurse-station/desensitization/new')
  await page.getByPlaceholder('例如：面包、小麦、牛乳酪蛋白').fill('牛肉')
  await expect(page.getByText('可带入的历史记录')).toBeVisible()
  await expect(page.locator('.desensitization-history-preview')).toContainText('找到 3 条')
  await page.screenshot({path:'test-results/desensitization-history-carry-in-375x667.png',fullPage:true})
  await page.getByRole('button',{name:'开始观察'}).click()
  const token=pageTokens.get(page)??'',list=await page.request.get(`/api/desensitization-tests?memberId=${encodeURIComponent(await currentMemberId(page))}`,{headers:{Authorization:`Bearer ${token}`}}),data=await list.json()
  expect(data.tasks[0].linkedRecords).toHaveLength(3)
  expect(new Set(data.tasks[0].linkedRecords.map((item:{occurredAt:string})=>item.occurredAt)).size).toBe(3)
})

test('关联查询失败不会伪装成空结果且输入可原地重试', async ({ page }) => {
  await registerMember(page)
  await page.route('**/api/desensitization-tests/resolve',route=>route.fulfill({status:503,contentType:'application/json',body:JSON.stringify({error:{message:'关联查询暂时不可用'}})}))
  await page.goto('/nurse-station/desensitization/new')
  await page.getByPlaceholder('例如：面包、小麦、牛乳酪蛋白').fill('小麦')
  await expect(page.getByRole('alert')).toContainText('已有观察没有查找完成')
  await expect(page.getByText('尚无相关观察')).toHaveCount(0)
  await expect(page.getByRole('button',{name:'重试查找'}).last()).toBeVisible()
  await page.unroute('**/api/desensitization-tests/resolve')
  await page.getByRole('button',{name:'重试查找'}).last().click()
  await expect(page.getByRole('heading',{name:'食物与成分关联'})).toBeVisible()
  await expect(page.getByRole('button',{name:'开始观察'})).toBeVisible()
})
...
test('排敏记录未保存保护与草稿在刷新后继续', async ({ page }) => {
  ...
  await page.getByRole('tab', { name: '排敏测试' }).click()
  await page.getByRole('button', { name: '新增测试', exact: true }).click()
  await page.getByPlaceholder('例如：面包、小麦、牛乳酪蛋白').fill('鸡蛋')
  await page.getByRole('button', { name: '开始观察' }).click()
  await page.getByRole('dialog', { name: /鸡蛋来源.*变化与下一步/ }).getByLabel(/关闭鸡蛋来源.*变化与下一步/).click()
  await page.getByTestId('desensitization-card-鸡蛋来源').getByRole('button', { name: '记一笔' }).click()
  await page.getByRole('button', { name: '没症状', exact: true }).click()
  await page.getByRole('dialog', { name: /记一笔.*鸡蛋来源/ }).getByLabel(/关闭记一笔.*鸡蛋来源/).click()
  ...
  await page.reload()
  await page.getByRole('tab', { name: '排敏测试' }).click()
  await expect(page.getByTestId('desensitization-card-鸡蛋来源').getByText(/最近1次未见症状/)).toBeVisible()
})
