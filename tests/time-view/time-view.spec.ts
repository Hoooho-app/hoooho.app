...
test('five diet record kinds save through the real API and show only the concise success message', async ({ page }) => {
  ...
  const reactions = page.getByRole('group', { name: '观察到的情况' })
  await expect(reactions.getByRole('button', { name: '暂未发现' })).toHaveAttribute('aria-pressed', 'true')
  ...
  await page.getByRole('button', { name: '进食后有无异常 （可选）' }).click()
  await expect(page.getByRole('group', { name: '观察到的情况' }).getByRole('button', { name: '暂未发现' })).toHaveAttribute('aria-pressed', 'true')
  await page.getByRole('button', { name: '保存记录', exact: true }).click()
