...
test('symptom entry opens directly, extracts explicit facts, switches card density and saves', async ({ page }) => {
  ...
  await page.getByLabel('单日时间轴').getByRole('button', { name: '当前为列表视图，点击切换为缩略图视图' }).click()
  await expect(page.getByLabel('单日时间轴').getByRole('button', { name: '当前为缩略图视图，点击切换为列表视图' })).toBeVisible()
  await expect(page.locator('.journal-layout-notice')).toHaveText('已切换为缩略图视图')
  await expect(page.locator('.journal-time-view')).toHaveAttribute('data-layout-mode', 'thumbnail')
  await page.screenshot({ path: 'test-results/symptom-thumbnail-iphone-se.png' })
})
...
test('dense hours expand only as needed while all records and a routine remain reachable', async ({ page }) => {
  ...
  expect((await sheet.boundingBox())!.width).toBeGreaterThan(320)
...
test('records in the same minute form one readable group without overlap', async ({ page }) => {
  await page.clock.setFixedTime(new Date('2026-09-22T05:26:00.000Z'))
  await page.setViewportSize({ width: 375, height: 667 })
  await prepare(page, 'routine-child')
  const headers = { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json', 'X-Hoooho-Timezone': 'Asia/Shanghai' }
  ...
  await page.reload()
  const group = page.locator('[data-hour="10"] .journal-minute-group--shared[data-minute="15"]')
  await expect(group).toContainText('15分 · 同一分钟')
  await expect(group.locator('.journal-record')).toHaveCount(2)
...
test('abnormal ongoing sleep keeps the original record and can be corrected to a valid interval', async ({ page }) => {
  ...
  await expect(ongoing).toContainText('这次睡眠尚未补全时间')
  await expect(ongoing).toContainText('原始开始')
...
test('single-day timeline, direct layout switch, search entry, sort order, compact subject and summary entry', async ({ page }) => {
  let timelineRequests = 0
  page.on('request', (request) => { if (request.url().includes('/records?view=time')) timelineRequests += 1 })
  await prepare(page)
  ...
  const layoutSwitch = page.getByRole('button', { name: '当前为列表视图，点击切换为缩略图视图' })
  ...
  const navigationLayout = await page.locator('.journal-date-navigation').evaluate((navigation) => {
    const controls = [...navigation.querySelectorAll(':scope > label, :scope > .journal-yesterday-entry, :scope > button, :scope > .journal-layout-switch')].map((element) => element.getBoundingClientRect())
    const search = document.querySelector('[aria-label="搜索健康随身记"]')!.getBoundingClientRect()
    const view = document.querySelector('[aria-label^="当前为列表视图"]')!.getBoundingClientRect()
    const sort = document.querySelector('[aria-label^="记录顺序："]')!.getBoundingClientRect()
    ...
    return { height: box.height, sameRow: controls.every((item) => item.top >= box.top && item.bottom <= box.bottom), ordered: search.right <= view.left && view.right <= sort.left, viewTarget: { width: view.width, height: view.height }, dateFits: dateBox.left >= previous.right && dateBox.right <= next.left, pageOverflow: document.documentElement.scrollWidth > document.documentElement.clientWidth }
  })
  expect(navigationLayout).toEqual({ height: 48, sameRow: true, ordered: true, viewTarget: { width: 44, height: 44 }, dateFits: true, pageOverflow: false })
  ...
  const recordIdsBeforeLayoutSwitch = await page.locator('.journal-record').evaluateAll((records) => records.map((record) => (record as HTMLElement).dataset.recordId))
  await layoutSwitch.click()
  await expect(page.locator('.journal-time-view')).toHaveAttribute('data-layout-mode', 'thumbnail')
  await expect(page.locator('.journal-layout-notice')).toHaveText('已切换为缩略图视图')
  await expect(page.locator('.journal-record--thumbnail')).toHaveCount(recordIdsBeforeLayoutSwitch.length)
  expect(await page.locator('.journal-record').evaluateAll((records) => records.map((record) => (record as HTMLElement).dataset.recordId))).toEqual(recordIdsBeforeLayoutSwitch)
  await expect(page.locator('.journal-day-picker')).toHaveText(dateBeforeLayoutSwitch)
  expect(timelineRequests).toBe(requestsBeforeLayoutSwitch)
  const thumbnailSwitch = page.getByRole('button', { name: '当前为缩略图视图，点击切换为列表视图' })
  await thumbnailSwitch.click()
  await page.getByRole('button', { name: '当前为列表视图，点击切换为缩略图视图' }).click()
  await expect(page.locator('.journal-layout-notice')).toHaveText('已切换为缩略图视图')
  await expect(page.locator('.journal-layout-notice')).toHaveCount(1)
  await expect(page.locator('.journal-layout-notice')).toHaveCount(0, { timeout: 2200 })
  const anchorRecord = page.locator('.journal-record').nth(5)
  await anchorRecord.scrollIntoViewIfNeeded()
  const anchorBefore = await anchorRecord.evaluate((record) => ({ id: (record as HTMLElement).dataset.recordId, top: record.getBoundingClientRect().top }))
  await page.getByRole('button', { name: '当前为缩略图视图，点击切换为列表视图' }).evaluate((button: HTMLButtonElement) => button.click())
  ...
  expect(Math.abs(anchorAfter.top - anchorBefore.top)).toBeLessThanOrEqual(100)
  expect(timelineRequests).toBe(requestsBeforeLayoutSwitch)
...
for (const width of [390, 430, 1280, 1440]) test(`layout remains usable at ${width}px`, async ({ page }) => {
  ...
  if (width === 1440) await page.screenshot({ path: 'test-results/health-journal-thumbnail-desktop-1440.png', fullPage: true })
})
