...
test('compact hour cells stop at now, persist routines and convert confirmation into one actual record', async ({ page }) => {
  ...
  await expect(page.locator('.journal-hour-group')).toHaveCount(1)
  await expect(page.locator('[data-hour="13"] .journal-timeline-row--now > time')).toHaveText('13:26')
  await expect(page.locator('[data-hour="13"] .journal-now-cell svg')).toBeVisible()
  expect(await page.locator('.journal-hour-group').evaluateAll((groups) => groups.every((group) => Number((group as HTMLElement).dataset.hour) <= 13))).toBe(true)
  ...
  await expect(page.locator('.routine-toolbar')).toContainText('全天时间轴')
  ...
  await page.setViewportSize({ width: 1440, height: 900 })
  await expect(page.locator('.journal-hour-group')).not.toHaveCount(24)
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
...
test('today excludes future routine points and projects one cross-night sleep summary', async ({ page }) => {
  ...
  await expect(page.locator('.journal-sleep-row--segment')).toHaveCount(9)
  const summary = page.locator('.journal-sleep-row--summary')
  await expect(summary).toHaveCount(1)
  await expect(summary).toContainText('睡眠 11小时28分')
  await expect(summary).toContainText('按作息推算')
  await expect(page.locator('.journal-timeline-row[data-time="08:28"]')).toContainText('睡眠 11小时28分')
...
test('dense hours keep every record expanded inside one growing hour cell', async ({ page }) => {
  ...
  await expect(hour.locator('.journal-timeline-row > time')).toHaveText(['06:45', '06:35', '06:30', '06:25', '06:15', '06:05'])
  await expect(hour.locator('.journal-timeline-row').first()).toHaveClass(/cell-start/)
  await expect(hour.locator('.journal-timeline-row').last()).toHaveClass(/cell-end/)
  expect(await hour.evaluate((node) => getComputedStyle(node).overflowY)).not.toBe('scroll')
...
test('sleep prompt starts one persistent session, restores after reload and ends it', async ({ page }) => {
  ...
  const activeSleep = page.locator('.journal-sleep-row:not(.journal-sleep-row--routine)').first()
  ...
  await expect(page.locator('.journal-sleep-row--summary:not(.journal-sleep-row--routine)')).toBeVisible()
})
...
test('records in the same minute share one hour cell with aligned times and no overlap', async ({ page }) => {
  ...
  await page.reload()
  const group = page.locator('[data-hour="10"]')
  await expect(group.locator('.journal-timeline-row[data-time="10:15"]')).toHaveCount(3)
  for (const [content] of sameMinuteRecords) await expect(group.getByText(content, { exact: true })).toBeVisible()
  const cards = await group.locator('.journal-timeline-row[data-time="10:15"] .journal-record').evaluateAll((nodes) => nodes.map((node) => node.getBoundingClientRect()).map((box) => ({ top: box.top, bottom: box.bottom })))
...
test('abnormal ongoing sleep keeps the original record and can be corrected to a valid interval', async ({ page }) => {
  ...
  await page.reload()
  await expect(page.locator('.journal-sleep-row--open')).toHaveCount(0)
  ...
  const ongoing = page.locator('.journal-sleep-row--open')
  await expect(ongoing).toContainText('结束时间未填写')
...
test('single-day timeline keeps the frozen top controls, unified hour cells and stable sort order', async ({ page }) => {
  ...
  const navigationLayout = await page.locator('.journal-date-navigation').evaluate((navigation) => {
    ...
    const search = navigation.querySelector('[aria-label="搜索健康随身记"]')!.getBoundingClientRect()
    ...
    return { height: box.height, sameRow: controls.every((item) => item.top >= box.top && item.bottom <= box.bottom), ordered: search.right <= sort.left, searchTarget: { width: search.width, height: search.height }, dateFits: dateBox.left >= previous.right && dateBox.right <= next.left, pageOverflow: document.documentElement.scrollWidth > document.documentElement.clientWidth }
  })
  expect(navigationLayout).toEqual({ height: 48, sameRow: true, ordered: true, searchTarget: { width: 32, height: 40 }, dateFits: true, pageOverflow: false })
  ...
  expect(await page.locator('.journal-record-content').first().evaluate((element) => ({ whiteSpace: getComputedStyle(element).whiteSpace, oneLine: element.scrollHeight <= element.clientHeight + 1 }))).toEqual({ whiteSpace: 'normal', oneLine: true })
  ...
  await expect(morning.locator('.journal-timeline-row > time')).toHaveText(['09:45', '09:30', '09:10'])
  await expect(morning.getByRole('button', { name: /还有 \d+ 条/ })).toHaveCount(0)
  ...
  await expect(page.locator('[data-hour="9"] .journal-timeline-row > time')).toHaveText(['09:10', '09:30', '09:45'])
