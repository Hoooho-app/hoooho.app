...
test('empty today shows one contextual prompt and current-time marker without the old empty state', async ({ page }) => {
  ...
  await expect(page.locator('.journal-memory-cursor')).toHaveText('_')
  await expect(page.locator('.journal-memory-typewriter')).toHaveAttribute('aria-label', '日常记录可以帮助补全孩子的健康情况')
  await expect(page.locator('.journal-memory-action')).toHaveCSS('background-color', 'rgba(27, 122, 110, 0.075)')
  await expect.poll(() => page.locator('.journal-memory-typed-copy').textContent()).not.toBe('')
  const promptLayout = await page.locator('.journal-memory-prompt').evaluate((card) => ({ height: card.getBoundingClientRect().height, overflows: document.documentElement.scrollWidth > document.documentElement.clientWidth }))
  expect(promptLayout.height).toBeLessThanOrEqual(238)
