...
test('symptom entry opens directly, extracts explicit facts and returns to the unified timeline', async ({ page }) => {
  ...
  await expect(form.getByLabel('当前发生时间')).toBeInViewport()
...
test('negated fever stays absent and the compact symptom fields persist', async ({ page }) => {
  ...
  await openSymptom(page)
  const form = page.getByRole('dialog', { name: '记录症状' })
  await expect(form.getByRole('button', { name: '现在', exact: true })).toHaveAttribute('aria-pressed', 'true')
  await expect(form.getByLabel('当前发生时间')).toHaveText(/^\d{2}:\d{2}:\d{2}$/)
  ...
  await expect(form.getByRole('button', { name: /补充信息/ })).toContainText('有些影响')
  await form.getByRole('button', { name: '保存', exact: true }).click()
  await expect(page.getByText('已记录', { exact: true })).toBeVisible()
...
test('a historical symptom save switches to its occurrence day and highlights the real saved record', async ({ page }) => {
  ...
  await expect(form.getByRole('button', { name: '指定时间', exact: true })).toHaveAttribute('aria-pressed', 'true')
  await expect(form.getByLabel('指定日期和时间')).toHaveValue(new RegExp(`^${historical.day}T`))
  await form.getByLabel('指定日期和时间').fill(historical.input)
  await form.getByRole('button', { name: '保存', exact: true }).click()
...
test('specified occurrence stays fixed while editing and rejects a future time beside the field', async ({ page }) => {
  ...
  await form.getByRole('button', { name: '指定时间', exact: true }).click()
  ...
  const input = form.getByLabel('指定日期和时间')
