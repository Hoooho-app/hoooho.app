...
test('symptom entry opens directly, extracts explicit facts and returns to the unified timeline', async ({ page }) => {
  ...
  await expect(form.getByRole('heading', { name: '哪里不舒服？', exact: true })).toBeVisible()
  await expect(form.getByRole('button', { name: '语音输入症状', exact: true })).toHaveCount(0)
  await expect(form.getByLabel('哪里不舒服')).toHaveCSS('font-size', '16px')
  expect(await form.getByLabel('哪里不舒服').evaluate((field) => field.getBoundingClientRect().height)).toBe(88)
  await form.getByPlaceholder('描述症状和变化，例如：左肘窝发红、发痒').fill('昨晚左肘窝有点发红，也很痒')
  ...
  const attachment = form.getByRole('button', { name: /照片与附件.*添加/ })
  const supplement = form.getByRole('button', { name: /补充信息.*展开/ })
  await expect(attachment).toBeInViewport()
  await expect(supplement).toBeInViewport()
  expect(await form.locator('.symptom-card-action').evaluateAll((buttons) => buttons.every((button) => button.getBoundingClientRect().height >= 44))).toBe(true)
  ...
  await expect(form.locator('.occurrence-time-control > span')).toHaveText(/^今天 \d{2}:\d{2} ›$/)
  expect(await form.locator('.occurrence-time-field').evaluate((field) => {
    ...
    return { aligned: Math.abs(label.top + label.height / 2 - control.top - control.height / 2) < 1, accessibleHeight: control.height >= 44 }
  })).toEqual({ aligned: true, accessibleHeight: true })
...
test('symptom attachments stay as six thumbnails on one row', async ({ page }) => {
  ...
  await page.route('**/api/quick-records/*/photos', (route) => route.request().method() === 'POST' ? route.fulfill({ status: 503, contentType: 'application/json', body: JSON.stringify({ error: { message: '上传暂时失败' } }) }) : route.continue(), { times: 1 })
  ...
  await expect(form.getByRole('button', { name: /重试上传/ })).toHaveCount(1)
  await expect(form.getByRole('button', { name: '保存', exact: true })).toBeDisabled()
  await form.getByRole('button', { name: /重试上传/ }).click()
  await expect(form.locator('.quick-record-photo[data-status="uploaded"]')).toHaveCount(6)
  await expect(form.getByRole('button', { name: '保存', exact: true })).toBeEnabled()
  await expect(form.getByText('6 张照片', { exact: true })).toBeVisible()
...
test('confirmed symptom visual states preserve location edits, time and supplement facts', async ({ page }) => {
  await page.setViewportSize({ width: 375, height: 667 })
  await prepare(page)
  await openSymptom(page)
  const form = page.getByRole('dialog', { name: '记录症状' })
  ...
  await expect(form.getByRole('textbox', { name: '发生时间' })).toBeInViewport()
  await expect(form.getByRole('button', { name: '保存', exact: true })).toBeInViewport()
  await page.screenshot({ path: 'outputs/symptom-record-initial-iphone-se.png' })
  ...
  await picker.getByPlaceholder(/搜索头/).fill('肘')
  await picker.getByRole('button', { name: '左肘', exact: true }).click()
  await picker.getByRole('button', { name: '右肘', exact: true }).click()
  await picker.getByRole('button', { name: /完成并返回症状记录/ }).click()
  await expect(form.locator('.symptom-location-tags > span')).toHaveCount(2)
  await expect(form.getByText('左肘 · 1号区域', { exact: true })).toBeVisible()
  await expect(form.getByText('右肘 · 2号区域', { exact: true })).toBeVisible()
  ...
  await form.locator('input[type="file"]').setInputFiles('src/assets/health-events/quick-record/symptom.webp')
  await expect(form.getByText('1 张照片', { exact: true })).toBeVisible()
  await expect(form.locator('.quick-record-photo[data-status="uploaded"]')).toHaveCount(1)
  await form.getByRole('button', { name: '查看照片 1', exact: true }).click()
  await expect(page.getByRole('dialog', { name: '照片预览' })).toBeVisible()
  await page.getByRole('button', { name: '关闭大图预览' }).click()
  await page.screenshot({ path: 'outputs/symptom-record-filled-iphone-se.png' })
...
test('failed symptom save keeps the complete draft and allows retry', async ({ page }) => {
  ...
  await openSymptom(page)
  const form = page.getByRole('dialog', { name: '记录症状' })
  await form.getByLabel('哪里不舒服').fill('保存失败后仍需保留的皮疹')
  ...
  await expect(form.getByLabel('哪里不舒服')).toHaveValue('保存失败后仍需保留的皮疹')
  await expect(form.getByLabel('手动补充症状部位')).toHaveValue('手臂')
  await expect(form.getByRole('button', { name: '保存', exact: true })).toBeEnabled()
...
test('manual record button opens the four-class hub and symptom continues directly', async ({ page }) => {
  ...
  const form = page.getByRole('dialog', { name: '记录症状' })
  await expect(form).toBeVisible()
  await expect(form.getByLabel('哪里不舒服')).toBeVisible()
  await expect(form.getByRole('button', { name: '保存', exact: true })).toBeInViewport()
...
test('compact hour cells stop at now, persist routines and convert confirmation into one actual record', async ({ page }) => {
  ...
  await expect(setup.getByLabel('夜间睡眠通常入睡')).toHaveCount(0)
  await sleepSwitch.click()
  await expect(setup.getByLabel('夜间睡眠通常入睡')).toHaveValue('21:00')
  await expect(setup.getByLabel('夜间睡眠通常醒来')).toHaveValue('08:28')
  await sleepSwitch.click()
...
test('custom routines use a focused naming flow, preserve settings and reject duplicate names', async ({ page }) => {
  await page.clock.setFixedTime(new Date('2026-09-22T05:26:00.000Z'))
  await page.setViewportSize({ width: 375, height: 667 })
  await prepare(page, 'routine-child')
  ...
  await setup.getByRole('button', { name: '添加自定义作息' }).click()
  let nameSheet = page.getByRole('dialog', { name: '添加自定义作息' })
  const addButton = nameSheet.getByRole('button', { name: '添加', exact: true })
  await expect(nameSheet.getByLabel('名称')).toBeFocused()
  await expect(addButton).toBeDisabled()
  await page.setViewportSize({ width: 375, height: 420 })
  await expect(nameSheet.getByLabel('名称')).toBeInViewport()
  await expect(addButton).toBeInViewport()
  expect(await nameSheet.evaluate((sheet) => sheet.scrollWidth <= sheet.clientWidth && document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  await page.setViewportSize({ width: 375, height: 667 })
  await nameSheet.getByLabel('名称').fill('早餐')
  await addButton.click()
  await expect(nameSheet.getByText('已存在同名作息', { exact: true })).toBeVisible()
  await nameSheet.getByLabel('名称').fill('户外散步')
  await page.screenshot({ path: 'outputs/routine-name-add-iphone-se.png' })
  await addButton.click()
  setup = page.getByRole('dialog', { name: '设置日常作息' })
  await expect(setup.getByRole('switch', { name: '户外散步作息' })).toHaveAttribute('aria-checked', 'true')
  await setup.getByLabel('户外散步开始时间').fill('10:00')
  await setup.getByLabel('户外散步结束时间').fill('10:30')

  await setup.getByRole('button', { name: '修改户外散步名称' }).click()
  nameSheet = page.getByRole('dialog', { name: '编辑名称' })
  await nameSheet.getByLabel('名称').fill('取消后的名称')
  await nameSheet.getByRole('button', { name: '取消' }).click()
  setup = page.getByRole('dialog', { name: '设置日常作息' })
  await expect(setup.getByText('户外散步', { exact: true })).toBeVisible()
  await setup.getByRole('button', { name: '修改户外散步名称' }).click()
  nameSheet = page.getByRole('dialog', { name: '编辑名称' })
  await nameSheet.getByLabel('名称').fill('周末户外散步')
  await page.screenshot({ path: 'outputs/routine-name-edit-iphone-se.png' })
  await nameSheet.getByRole('button', { name: '保存名称' }).click()
  setup = page.getByRole('dialog', { name: '设置日常作息' })
  await expect(setup.getByText('名称已更新', { exact: true })).toBeVisible()
  await expect(setup.getByLabel('周末户外散步开始时间')).toHaveValue('10:00')
  await expect(setup.getByLabel('周末户外散步结束时间')).toHaveValue('10:30')
  for (const width of [390, 430]) {
    await page.setViewportSize({ width, height: 667 })
    expect(await setup.evaluate((sheet) => sheet.scrollWidth <= sheet.clientWidth && document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  }
  await page.setViewportSize({ width: 375, height: 667 })
  await page.screenshot({ path: 'outputs/routine-setup-custom-iphone-se.png' })
  await setup.getByRole('button', { name: '保存并执行' }).click()
  await expect(page.getByText('已保存并执行', { exact: true })).toBeVisible()
  const customTrack = page.locator('.journal-activity-row--routine').filter({ hasText: '周末户外散步' })
  await expect(customTrack.first()).toBeVisible()
  const response = await page.request.get('/api/routines/routine-child?day=2026-09-22', { headers: { Authorization: `Bearer ${token}`, 'X-Hoooho-Timezone': 'Asia/Shanghai' } })
  const routineDay = await response.json()
  expect(routineDay.tracks).toHaveLength(1)
  expect(routineDay.tracks[0]).toMatchObject({ category: 'activity', title: '周末户外散步', time: '10:00', endTime: '10:30' })

  await page.getByRole('button', { name: '调整作息' }).click()
  setup = page.getByRole('dialog', { name: '设置日常作息' })
  await expect(setup.getByRole('button', { name: '停用日常作息' })).toHaveCount(0)
  const customSwitch = setup.getByRole('switch', { name: '周末户外散步作息' })
  await customSwitch.click()
  await expect(setup.getByLabel('周末户外散步开始时间')).toHaveCount(0)
  await customSwitch.click()
  await expect(setup.getByLabel('周末户外散步开始时间')).toHaveValue('10:00')
  await expect(setup.getByLabel('周末户外散步结束时间')).toHaveValue('10:30')
  await customSwitch.click()
...
test('a historical symptom save switches to its occurrence day and highlights the real saved record', async ({ page }) => {
  ...
  await openSymptom(page)
  const form = page.getByRole('dialog', { name: '记录症状' })
  await form.getByLabel('哪里不舒服').fill('历史补录定位验收')
...
test('specified occurrence stays fixed while editing and rejects a future time beside the field', async ({ page }) => {
  ...
  await openSymptom(page)
  const form = page.getByRole('dialog', { name: '记录症状' })
  await form.getByLabel('哪里不舒服').fill('指定时间校验记录')
  ...
  await expect(form.getByLabel('哪里不舒服')).toHaveValue('指定时间校验记录')
...
test('short keyboard viewport keeps direct symptom form actionable and closeable', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: '哪里不舒服', exact: true }).fill('键盘布局验收')
...
test('free symptom narratives save without tags and location remains optional', async ({ page }) => {
  ...
  await expect(form.getByPlaceholder('例如：左肘窝', { exact: true })).toBeVisible()
  await expect(form.getByRole('button', { name: /选择部位/ })).toBeVisible()
  await expect(form.getByLabel('哪里不舒服')).not.toBeFocused()
  await form.getByLabel('哪里不舒服').focus()
  ...
  for (const narrative of ['感冒', '手冰凉']) {
    ...
    await expect(form.getByText(/正在为：/)).toHaveCount(0)
    await form.getByLabel('哪里不舒服').fill(narrative)
    await expect(form.getByText(/已整理|暂未生成摘要|暂时无法整理/)).toBeVisible()
    await expect(form.getByRole('button', { name: '保存', exact: true })).toBeEnabled()
    if (narrative === '感冒') {
      await form.getByLabel('手动补充症状部位').fill('1')
      await form.getByLabel('手动补充症状部位').fill('')
    }
    await form.getByRole('button', { name: '保存', exact: true }).click()
    await createDespiteDuplicateIfNeeded(page)
    await expect(page.getByText('已记录', { exact: true })).toBeVisible()
