...
try {
  ...
  const manualWidth = await page.getByRole('button', { name: '记一下', exact: true }).evaluate((element) => element.getBoundingClientRect().width)
  const quickWidth = await page.getByRole('button', { name: '快捷记录', exact: true }).evaluate((element) => element.getBoundingClientRect().width)
  assert.ok(manualWidth > quickWidth * 4)
  assert.equal(quickWidth, 52)
  ...
  await page.getByRole('button', { name: '运动', exact: true }).click()
  ...
  await row.waitFor()
  assert.equal(await row.getByText('运动', { exact: true }).count(), 1)
  ...
  await page.getByRole('button', { name: '搜索健康随身记', exact: true }).click()
  const search = page.getByRole('searchbox', { name: '搜索健康随身记', exact: true })
  await search.waitFor()
  assert.equal(await search.getAttribute('placeholder'), '输入名称，即可查看发生时间')
  assert.equal(await page.locator('.journal-search-body').textContent(), '')
  await search.fill('合成测试记录')
  await page.getByText('找到 1 条相关随记', { exact: true }).waitFor()
